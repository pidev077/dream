    ...
    Bears.MasonryHybridSaveGridHandle = function(obj) {
        ...
        button_save.on('click', function(e) {
            ...
            $.ajax({
                ...
                data: { action: '_dream_save_gridmap_masonryhybrid', params: { gridname: grid_name, gridmap: grid_map } },
        ...
        $('body').on('input', 'input[data-search-ajax-result]', function(e) {
            ...
            if (this.value.length >= 2) {
                ...
                $.ajax({
                    type: 'POST',
                    url: BtPhpVars.ajax_url,
                    data: { action: '_dream_search_data_ajax', search_data: s },
                    success: function(result) {
                        try {
                            var result_obj = JSON.parse(result);
				...
				$thisEl.on({
					'_load_content_by_id' (e, id) {
						$.ajax({
							type: 'POST',
							url: BtPhpVars.ajax_url,
							data: {action: '_dream_semon_load_media_template', pid: id},
							success (res) {
								console.log(res);
