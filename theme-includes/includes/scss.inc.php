...
class scss_server {
	...
	protected function inputName() {
		...
		switch (true) {
			case isset($_GET['p']):
				return $_GET['p'];
			case isset($server['PATH_INFO']):
				return $server['PATH_INFO'];
			case isset($server['DOCUMENT_URI']):
				return substr($server['DOCUMENT_URI'], strlen($server['SCRIPT_NAME']));
		}
	}
	...
	protected function getModifiedSinceHeader()
	{
		...
		if (isset($server['HTTP_IF_MODIFIED_SINCE'])) {
			$modifiedSince = $server['HTTP_IF_MODIFIED_SINCE'];
