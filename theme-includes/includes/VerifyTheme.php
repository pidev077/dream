...
function isInstallationLegit( $data = false ) {
  ...
  $server_name = empty($server['SERVER_NAME']) ?
...
class VerifyTheme {
    ...
    public function verifytheme_settings_page(){
        ...
        $other_attributes = '';
        $register_button_text = __( 'Register your theme', 'dream' );
        if ( $toolkitData && $installationLegit ){
          $other_attributes = 'disabled';
          $register_button_text = __( 'Activated on this domain', 'dream' );
        ...
        <div class="wrap verifytheme_wrap">
            <form class="verifytheme_settings_form" method="post" action="options.php">
              ...
              <p class="change_license_wrap">
                <input name="change_license_tmp" onclick="document.getElementById('change_license_btn').click();" id="change_license_tmp" class="button" value="<?php esc_attr_e('Deregister your product','dream'); ?>" type="button">
              </p>
            <?php endif; ?>
            </form>
            <form style="display: none" id="change_license_form" method="POST">
              <button id="change_license_btn" type="submit" class="button button-primary" name="change_license"><?php echo esc_html__( 'Deregister your product', 'dream' ); ?></button>
            </form>
        </div>
        <?php
    }
    ...
    public function verifytheme_sanitize( $input )
    {
        ...
        if(!empty($message)):
          ...
          if ( ! $already_in_use ):
            ...
            $message .= sprintf(wp_kses( __( 'This product is in use on another domain: <span>%s</span><br />', 'dream' ), array( 'span' => array(), 'br' => array() ) ), $connected_domain );
            $message .= sprintf(esc_html__('Are you using this theme for a new site? Please purchase a %s ', 'dream' ), '<a tabindex="-1" href="' . esc_url( 'http://themeforest.net/cart/add_items?ref=bearsthemes&item_ids=' ) .ITEM_ID.'" target="_blank">'.esc_html__('new license','dream').'</a>');
            if(!$register_error):
              add_settings_error(
                  'verifytheme_settings',
                  esc_attr( 'settings_updated' ),
    ...
    public function print_section_info()
    {
        printf(
            ...
            esc_html__('Themeforest provides purchase code for each theme you buy, and you’ll need it to verify and register your product (and to receive theme support).','dream'),esc_html__('To download your purchase code, simply follow these steps at ','dream'), esc_url('//bearsthemes.com/product-registration/'), esc_html__('here','dream')
        );
    }
    /**
    ...
    public function verifytheme_purchase_code_callback()
    {
        printf(
            ...
            isset( $this->options['purchase_code'] ) ? esc_attr( $this->options['purchase_code']) : '', esc_html__('Please insert your Envato purchase code. ','dream'), esc_url('//bearsthemes.com/product-registration/'), esc_html__('More info','dream')
        );
    }
}
