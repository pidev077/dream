...
if( ! function_exists( 'dream_get_the_archive_title' ) ) :
	/**
	 * Get the archive title
	 */
	function dream_get_the_archive_title() {
		if ( is_category() ) {
			...
			$title = get_the_date( _x( 'Y', 'yearly archives date format', 'dream' ) );
		} elseif ( is_month() ) {
			$title = get_the_date( _x( 'F Y', 'monthly archives date format', 'dream' ) );
		} elseif ( is_day() ) {
			$title = get_the_date( _x( 'F j, Y', 'daily archives date format', 'dream' ) );
			...
			$title = sprintf( esc_html__( '%1$s: %2$s', 'dream' ), $tax->labels->singular_name, single_term_title( '', false ) );
		} else {
			$title = esc_html__( 'Archives', 'dream' );
		}
		 ...
		 */
		return apply_filters( 'dream_get_the_archive_title', $title );
	}
endif;

if ( ! function_exists( 'dream_load_decentralize_setting' ) ) :

	function dream_load_decentralize_setting( $params = array(), $type = 'parent' ) {
...
if ( ! function_exists( 'dream_list_pages' ) ):
	/**
	 * get an array of all pages
	 */
	function dream_list_pages() {
		$pages = get_pages();
		$result = array();
		$result[0] = esc_html__('None', 'dream');
...
if ( ! function_exists( 'dream_return_memory_size' ) ) :
	/**
	 * print theme requirements page
	 *
	 * @param string $size
	 */
	function dream_return_memory_size( $size ) {
...
if ( ! function_exists( 'dream_logo' ) ):
	/**
	 * Display theme logo
	 *
	 * @param boolean $wrap
	 */
	function dream_logo( $wrap = false ) {
		$dream_logo_settings['logo']['selected_value']   = 'text';
		$dream_logo_settings['logo']['text']['title']    = get_bloginfo( 'name' );
		$dream_logo_settings['logo']['text']['subtitle'] = esc_html__('', 'dream');
		$TBFW = defined( 'FW' );   if ($TBFW ) {
			$dream_logo_settings = fw_get_db_customizer_option( 'logo_settings' );
		}

		$empty_logo = false;
		if( $dream_logo_settings['logo']['selected_value'] == 'image' ) {
			if( empty($dream_logo_settings['logo']['image']['image_logo']) ) {
				$empty_logo = true;
			}
		} else {
			if( empty($dream_logo_settings['logo']['text']['title']) && empty($dream_logo_settings['logo']['text']['subtitle']) ) {
				$empty_logo = true;
			}
		}

		// logo sticky
		$img_sticky_logo = $dream_logo_src_sticky_header = '';
		$TBFW = defined( 'FW' );   if ($TBFW ) {
			$dream_logo_src_sticky_header = fw_get_db_customizer_option('header_settings/enable_sticky_header/fw-sticky-header/image_logo_sticky/url');
		}

		$trim_dream_logo_src_sticky_header = trim($dream_logo_src_sticky_header);
		if(!empty($trim_dream_logo_src_sticky_header)) {
			$img_sticky_logo = sprintf('<img src="%s" alt="%" class="sticky-logo">', $dream_logo_src_sticky_header, esc_html__('Logo', 'dream'));
		}
		// echo '<pre>'; print_r($dream_logo_sticky_header); echo '</pre>';
		...
		if( !$empty_logo ) : ?>
			<div class="fw-wrap-logo">
				...
				<?php endif; ?>
					...
					<?php if ( $dream_logo_settings['logo']['selected_value'] == 'image' ) :
						$image_logo = $dream_logo_settings['logo']['image']['image_logo'];
						if ( ! empty( $image_logo ) ) : ?>
						...
						if ( $dream_logo_settings['logo']['text']['title'] != '' ) : ?>
							<a href="<?php echo esc_url( home_url('/') ); ?>" class="fw-site-logo">
								<strong class="site-title" itemprop="headline"><?php echo "{$dream_logo_settings['logo']['text']['title']}"; ?></strong>
								<?php if ( $dream_logo_settings['logo']['text']['subtitle'] != '' ) : ?>
									<span class="site-description" itemprop="description"><?php echo "{$dream_logo_settings['logo']['text']['subtitle']}"; ?></span>
								<?php endif; ?>
							</a>
						<?php endif;
					endif; ?>
...
if( ! function_exists('dream_header_mobile_menu') ) :
	function dream_header_mobile_menu() {
		$_FW = defined( 'FW' );
		if ($_FW ) {
			$dream_logo_settings = fw_get_db_customizer_option( 'logo_settings' );
...
if ( ! function_exists( 'dream_header' ) ) :
	/**
	 * Display theme header
	 */
	function dream_header() {
		$dream_header_page_custom_settings = dream_get_options_header();

		$dream_header_settings = defined( 'FW' ) ? $dream_header_page_custom_settings : array();
		$dream_header_type     = isset( $dream_header_settings['header_type_picker']['header_type'] ) ? $dream_header_settings['header_type_picker']['header_type'] : 'header-1';


//		$dream_header_settings = defined( 'FW' ) ? fw_get_db_customizer_option( 'header_settings' ) : array();
//		$dream_header_type = isset( $dream_header_settings['header_type_picker']['header_type'] ) ? $dream_header_settings['header_type_picker']['header_type'] : 'header-1';

		/* overide version header use $_GET */
		if(isset($_GET['header_ver']) && ! empty($_GET['header_ver'])) $dream_header_type = $_GET['header_ver'];

		/* load header template */
		get_template_part( 'templates/headers/'.$dream_header_type );
	}
endif;

if( !function_exists('dream_scss_handle') ) :

	function dream_scss_handle( $scss_content_string = '' ) {
		...
		$style_path = get_template_directory() . '/assets/css/dream.css';
...
if(!function_exists('dream_scan_enqueue_google_font')) :
	/*
	 * scan google font from theme setting
	 */
	function dream_scan_enqueue_google_font( $settings = array() ) {
		foreach( $settings as $key => $item ) :
			if( isset( $item['google_font'] ) ) :
				...
				if( is_array( $item) )
					dream_scan_enqueue_google_font( $item );
			endif;
		endforeach;
	}
endif;

if(! function_exists('dream_general_recipe_options')) :
	function dream_general_recipe_options() {
...
if( ! function_exists( 'dream_general_posts_options' ) ) :
	/**
	 * return theme general posts options
	 */
	function dream_general_posts_options() {
...
if ( ! function_exists( 'dream_get_featured_posts' ) ) :
	/**
	 * Getter function for Featured Content Plugin.
	 *
	 * @param array|bool $posts Array of featured posts, otherwise false.
	 */
	function dream_get_featured_posts() {
		return apply_filters( 'dream_get_featured_posts', array() );
	}
endif;

if(!function_exists('dream_extend')) :
	/**
 	 * extend
 	 */
	function dream_extend($base = array(), $replacements = array()) {
...
if(!function_exists('dream_get_settings_by_post_id')) :
	/*
	 * Get setting post by id
	 */
	function dream_get_settings_by_post_id($post_id) {
		$post_settings = array();
...
if( ! function_exists( 'dream_listing_post_media' ) ) :
	/**
	 * return listing post options
	 *
	 * @param integer $post_id
	 */
	function dream_listing_post_media($post_id) {
		global $post;
		$wrap_title = 'h2';
		$image_size = $post_format = $layout_creative = '';

		$TBFW = defined( 'FW' );   if ($TBFW ) {
			$post_settings          = dream_get_settings_by_post_id($post_id);
		...
		$result = array(
			...
			'featured_image'	=> dream_get_image(get_post_thumbnail_id(), array('size' => $image_size)),
			...
			'views'						=> dream_get_post_views($post_id),
			'readmore' 				=> "<a href='". get_permalink() ."' class='post-readmore'>". esc_html__('Read More', 'dream') ."</a>",
		);
		...
		if (in_array($post_format, $post_format_arr)) {
			if(function_exists("dream_get_{$post_format}")) {
				...
				$result[$post_format] = call_user_func_array("dream_get_{$post_format}", array($post_settings["post_{$post_format}_tab"]));
			}
		}
...
if( ! function_exists( 'dream_single_post_options' ) ) :
	/**
	 * return single post options
	 *
	 * @param integer $post_id
	 */
	function dream_single_post_options($post_id) {
		global $post;
...
if(!function_exists('dream_get_post_views')) :
	/**
 	 * dream_get_post_views
 	 * @param [int] $post_id
 	 */
	function dream_get_post_views($post_id = null) {
		$views = get_post_meta($post_id, 'fw_post_views', true);
		return !empty($views) ? $views : 0;
	}
endif;

if(!function_exists('dream_get_image')) :
	/*
 	 * get image post
 	 */
	function dream_get_image($attachment_id = null, $arg = null){
		if(empty($attachment_id)) return;

		$arg = dream_extend(array(
			'size' => 'medium-large',
			'icon' => false,
...
if(!function_exists('dream_get_video')) :
	/*
	 * get video post
	 */
	function dream_get_video($data = null){
				...
				if(!empty($video_src)){
					...
					$output = "<div class='post-video-wrap video-type-other'>
						<video controls>
							...
							". esc_html__('Your browser does not support the video tag.', 'dream') ."
						</video>
					</div>";
				}
				break;
...
if(!function_exists('dream_get_audio')) :
	/**
 	 * ge audio post
 	 */
	function dream_get_audio($data = null){
			...
			default:
				...
				$output = "<div class='post-video-wrap audio-type-other'>
					<audio controls>
						...
						". esc_html__('Your browser does not support the audio tag.', 'dream') ."
					</audio>
				</div>";
				break;
		}
...
if(!function_exists('dream_get_quote')) :
	/**
 	 * get quote post
 	 */
	function dream_get_quote($data = null){
...
if(!function_exists('dream_get_link')) :
	/**
 	 * get link post
 	 */
	function dream_get_link($data = null){
...
if(!function_exists('dream_get_gallery')) :
	function dream_get_gallery($data = null){
		...
		foreach($gallery_images as $item) {
			$image_item .= "<a href='{$item['url']}' class='gallery-item item'>". dream_get_image($item['attachment_id'], $data['img_arg']) ."</a>";
		}
...
if ( ! function_exists( 'dream_translate' ) ) :
	/**
	 * Return the content for translations plugins
	 *
	 * @param string $content
	 */
	function dream_translate( $content ) {
...
if ( ! function_exists( 'dream_get_shortcode_advanced_styles' ) ) :
	/**
	 ...
	 * @param array $atts
	 */
	function dream_get_shortcode_advanced_styles( $style, $atts = array('return_color' => false, 'general_options' => false, 'custom_meta' => '' ) ) {
		...
		if( isset($style['color-palette']['id']) && $style['color-palette']['id'] !== 'fw-custom'){
			if( $atts['return_color'] ){
				// get colors from db
				global $dream_color_settings;
				$advanced_styles .= 'color:'.$dream_color_settings[ $style['color-palette']['id'] ].';';
			}
			else {
...
if( ! function_exists( 'dream_responsive_heading_styles' ) ) :
	/**
	 * return text size styles
	 *
	 * @param array $atts
	 */
	function dream_responsive_heading_styles( $atts = array( 'styles' => array(), 'selector' => '', 'important' => false ) ) {
		$return_html = '';
...
if ( ! function_exists( 'dream_get_font_array' ) ) :
	/**
	 * get an array of fonts
	 *
	 * @param array $font_array
	 * @param array $dream_color_settings
	 */
	function dream_get_font_array( $font_array, $dream_color_settings ) {
		...
		elseif( isset($font_array['color-palette']['id']) && isset($dream_color_settings[$font_array['color-palette']['id']]) ){
            $return['color'] = $dream_color_settings[$font_array['color-palette']['id']];
		}
...
if ( ! function_exists( 'dream_get_remote_fonts' ) ) :
	/**
	 * Get remote fonts
	 *
	 * @param array $include_from_google
	 */
	function dream_get_remote_fonts( $include_from_google ) {
...
if( ! function_exists( 'dream_array_merge_recursive' ) ) :
	/**
	 ...
	 * @param array $b
	 */
	function dream_array_merge_recursive($a, $b) {
		...
		foreach (array_merge(array_keys($a), array_keys($b)) as $k) {
			if (
				...
				$a[$k] = dream_array_merge_recursive($a[$k], $b[$k]);
			} elseif (isset($b[$k])) {
				$a[$k] = $b[$k];
			}
		}
...
if ( ! function_exists( 'dream_is_page_url_excluded' ) ) :
	/**
	 * Check if is page is from excluded pages
	 */
	function dream_is_page_url_excluded() {
		...
		foreach ( $exception_urls as $url ){
			if ( strstr( $server['PHP_SELF'], $url) ) return true;
		}

		if ( strstr($server['QUERY_STRING'], 'feed=') ) return true;

		return false;
	}
endif;

if(!function_exists( 'dream_get_color_palette_color_and_class')) :
	/**
	 * Get colors
	 *
	 ...
	 * @param array $atts
	 */
	function dream_get_color_palette_color_and_class( $color_array, $atts = array('return_color' => false ) ) {
		...
		if ( $color_array['id'] == 'fw-custom' ) {
			...
			if( $atts['return_color'] ){
				// get colors from db
				global $dream_color_settings;
				$return['color'] = $dream_color_settings[ $color_array['id'] ];
			}
			else {
...
if(!function_exists('dream_scss_variables_handle')) :
	/**
 	 * build variable scss
 	 */
	function dream_scss_variables_handle($data = array()) {
...
if( !function_exists('dream_replace_http') ) :
    /**
     * Replace http with empty string in a URL
     * @param string $url
     */
    function dream_replace_http($url) {
        $url = trim($url);
        $url = trim($url, "/");
...
if( !function_exists('dream_change_original_link_with_cdn') ) :
    /**
     * Replace original link with a CDN link
     * @param string $url
     */
    function dream_change_original_link_with_cdn($url) {
        $final_url = $url;
        if( class_exists('WpFastestCache') ) {
            ...
            if ($cdn_values) {
                ...
                $std->cdnurl = dream_replace_http($std->cdnurl);

                // remove http from original url
                $url = dream_replace_http($url);
        ...
        elseif( function_exists('wp_cache_add_pages') ) {
            global $ossdlcdn;
            if( $ossdlcdn ) {
                $siteurl = dream_replace_http(get_option('siteurl'));

                $ossdl_off_cdn_url = get_option('ossdl_off_cdn_url');
                // remove http from CDN url
                $ossdl_off_cdn_url = trim($ossdl_off_cdn_url);
                $ossdl_off_cdn_url = trim($ossdl_off_cdn_url, "/");
                $ossdl_off_cdn_url = dream_replace_http($ossdl_off_cdn_url);
...
if ( ! function_exists( 'dream_button_class' ) ) :
	/**
	 * Display specific class for buttons - depends on theme
	 *
	 * @param string $style
	 */
	function dream_button_class( $style ) {
		if ( $style == 'fw-btn-1' ) {
			echo 'fw-btn-1';
...
if(!function_exists('dream_get_all_wordpress_menus')) :
	function dream_get_all_wordpress_menus(){
	    return get_terms( 'nav_menu', array( 'hide_empty' => true ) );
	}
endif;

if(!function_exists('dream_build_select_option_wordpress_menu')) :
	function dream_build_select_option_wordpress_menu() {
		$menus = dream_get_all_wordpress_menus();
...
if ( ! function_exists( 'dream_get_content_class' ) ) :
	/**
	 * Get content class when is full width or width sidebar
	 *
	 * @param string $parameter
	 * @param string $dream_sidebar_position
	 */
	function dream_get_content_class( $parameter, $dream_sidebar_position ) {
		$class = '';
		if ( $parameter == 'content' ) {
			if ( $dream_sidebar_position == 'left' || $dream_sidebar_position == 'right' ) {
			...
			if ( $dream_sidebar_position == 'left' ) {
				$class = 'sidebar-left';
			} elseif ( $dream_sidebar_position == 'right' ) {
				$class = 'sidebar-right';
			}
		}
		echo esc_attr($class);
...
if ( ! function_exists( 'dream_paging_navigation' ) ) :
	/**
	 * Display archive/category pagination
	 *
	 * @param object $wp_query
	 */
	function dream_paging_navigation( $wp_query = null ) {
		if ( ! $wp_query ) {
        ...
        $dream_pagination_type = function_exists('fw_get_db_customizer_option') ? fw_get_db_customizer_option('post_settings/blog_pagination', 'paging-navigation-type-1') : 'paging-navigation-type-1';
        if( $dream_pagination_type == 'paging-navigation-type-2' ) {
            $prev_text = esc_html__( 'Prev Page', 'dream' );
            $next_text = esc_html__( 'Next Page', 'dream' );
            $prev_icon = '<i class="ion-ios-arrow-thin-left"></i>';
            $next_icon = '<i class="ion-ios-arrow-thin-right"></i>';
        }
        else {
            $prev_text = esc_html__( 'Newer', 'dream' );
            $next_text = esc_html__( 'Older', 'dream' );
		...
		if ( $links ) : ?>
			<nav class="navigation paging-navigation <?php echo esc_attr($dream_pagination_type); ?>" role="navigation">
...
if(!function_exists('dream_get_footer_class')) :
	function dream_get_footer_class($echo = false) {
		$result = '';
...
if ( ! function_exists( 'dream_footer' ) ) :
	/**
	 * Display theme footer
	 */
	function dream_footer() {
		$dream_footer_settings = defined( 'FW' ) ? fw_get_db_customizer_option( 'footer_settings' ) : array();
		$bearsthemes_link	 = 'http://bearsthemespremium.com/';
		$show_footer_widgets = isset( $dream_footer_settings['show_footer_widgets']['selected_value'] ) ? $dream_footer_settings['show_footer_widgets']['selected_value'] : 'no';
		$copyright_position  = isset( $dream_footer_settings['copyright_position'] ) ? $dream_footer_settings['copyright_position'] : 'fw-copyright-center text-center';
		$copyright           = isset( $dream_footer_settings['copyright'] ) ? $dream_footer_settings['copyright'] : 'Copyright &copy;'. date("Y") .' <a href="'.$bearsthemes_link.'" rel="nofollow">Bearsthemes</a>. All Rights Reserved';

	$theme_footer  = array(
		'hidden_footer_default' => false,
		'btfb_footer'=>''
	);
	$theme_footer_filter = 	apply_filters( 'bears_before_footer_widgets', $theme_footer, $show_footer_widgets );

	echo $theme_footer_filter['btfb_footer'];
	if (!$theme_footer_filter['hidden_footer_default']) {
		if ( $show_footer_widgets == 'yes' ) :
...
if ( ! function_exists( 'dream_twitter_formating' ) ) :
	/**
	 ...
	 * @param string $user
	 */
	function dream_twitter_formating( $text, $user ) {
		$pattern = array(
			'/[^(:\/\/)](www\.[^ \n\r]+)/',
			'/(https?:\/\/[^ \n\r]+)/',
			'/@(\w+)/',
...
if ( ! function_exists( 'dream_get_instagram_photos' ) ):
	/**
	 ...
	 */
	function dream_get_instagram_photos( $username, $items = 9 ) {
		if ( false === ( $instagram = get_transient( 'instagram-photos-' . sanitize_title_with_dashes( $username ) . '-'.$items ) ) ) {
			...
			if ( is_wp_error( $connect ) ) {
				return new WP_Error( 'site_down', esc_html__( 'Unable to communicate with Instagram.', 'dream' ) );
			}

			if ( 200 != wp_remote_retrieve_response_code( $connect ) ) {
				return new WP_Error( 'invalid_response', esc_html__( 'Instagram did not return a 200.', 'dream' ) );
			}
			...
			if ( ! $instagram_array ) {
				return new WP_Error( 'bad_json', esc_html__( 'Instagram has returned invalid data.', 'dream' ) );
			}
...
if ( ! function_exists( 'dream_get_posts' ) ):
	/**
	 ...
	 * @param boolean $return_for_dream_image Return for dream_image function
	 ...
	 */
	function dream_get_posts( $args = null ) {
		$defaults = array(
			'sort'                => 'recent',
			'items'               => 5,
			'post_by_id'					=> array(),
		...
		if ( ! empty( $posts ) ) {
			foreach ( $posts as $post_elm ) {
				...
				if ( ! empty( $img ) ) {
					$fw_post_option[ $dream_count ]['post_img'] = $img;
				} else {
					$fw_post_option[ $dream_count ]['post_img'] = '';
				}

				if ( $date_post ) {
					$time_format                                = apply_filters( '_filter_widget_time_format', $date_format );
					$fw_post_option[ $dream_count ]['post_date_post'] = get_the_time( $time_format, $post_elm->ID );
				} else {
					$fw_post_option[ $dream_count ]['post_date_post'] = '';
				}

				$fw_post_option[ $dream_count ]['post_id']        		= $post_elm->ID;
				$fw_post_option[ $dream_count ]['post_class']        = ( is_singular() && $post->ID == $post_elm->ID ) ? 'current-menu-item post_' . $sort : 'post_' . $sort;
				$fw_post_option[ $dream_count ]['post_title']        = get_the_title( $post_elm->ID );
				$fw_post_option[ $dream_count ]['post_link']         = get_permalink( $post_elm->ID );
				$fw_post_option[ $dream_count ]['post_author_link']  = get_author_posts_url( get_the_author_meta( 'ID' ) );
				$fw_post_option[ $dream_count ]['post_author_name']  = get_the_author();
				$fw_post_option[ $dream_count ]['post_comment_numb'] = get_comments_number( $post_elm->ID );
				$fw_post_option[ $dream_count ]['post_excerpt']      = ( isset( $post ) ) ? get_the_excerpt() : '';
				$dream_count ++;
			}
			wp_reset_postdata();
		}
...
if ( ! function_exists( 'dream_single_post_title' ) ) :
	/**
	 ...
	 */
	function dream_single_post_title( $post_id, $post_type = 'post' ) {
		if ( ! defined( 'FW' ) ) {
		...
		if ( ( empty( $image ) && $overlay != 'yes' ) || ( isset( $posts_header_title['posts_title'] ) && $posts_header_title['posts_title'] != 'post_title' ) ) {
			$dream_blog_title = fw_get_db_settings_option('posts_settings/blog_title/selected', 'h2');
			$itemprop = 'headline';
			?>
			<<?php echo "{$dream_blog_title}"; ?> class="entry-title" itemprop="<?php echo esc_attr($itemprop); ?>"><?php the_title(); ?></<?php echo "{$dream_blog_title}"; ?>>
		<?php }
	}
endif;

if(!function_exists('dream_get_sidebars')) :
	function dream_get_sidebars($data = array()) {
...
if(!function_exists('dream_top_bar')) :
	function dream_top_bar() {
//		$dream_header_settings = defined( 'FW' ) ? fw_get_db_customizer_option( 'header_settings' ) : array();

		$dream_header_page_custom_settings = dream_get_options_header();
		$dream_header_settings             = defined( 'FW' ) ? $dream_header_page_custom_settings : array( 'header_type_picker' => array() );

		if(
			isset($dream_header_settings['enable_header_top_bar']) &&
			$dream_header_settings['enable_header_top_bar']['selected_value'] == 'yes'
		) {
			$sidebar_list = $dream_header_settings['enable_header_top_bar']['yes']['header_top_sidebar'];
...
if(!function_exists('dream_top_bar_mobi')) :
	function dream_top_bar_mobi() {
//		$dream_header_settings = defined( 'FW' ) ? fw_get_db_customizer_option( 'header_settings' ) : array();
		...
		if(
			isset($dream_header_settings['enable_header_top_bar_mobi']) &&
			$dream_header_settings['enable_header_top_bar_mobi']['selected_value'] == 'yes'
		) {
			$sidebar_list = $dream_header_settings['enable_header_top_bar_mobi']['yes']['header_top_mobi_sidebar'];
			$count = count($sidebar_list);
			$col = array(
				1 => 'col-md-12 col-sm-12 col-sx-12',
				2 => 'col-md-6 col-sm-12 col-sx-12',
...
if(! function_exists('dream_title_bar_default') ) :
	function dream_title_bar_default() {
		if(is_single()) return;
		...
		if( isset($term->taxonomy) ){
			...
			if( is_post_type_archive('product')){
				$title = esc_html__('Products', 'dream');
			}
			elseif( is_search() ){
				$title = esc_html__( 'Search results', 'dream' );
			}
			elseif( is_page() || is_single() ) {
				$title = get_the_title();
			}
			else{
				$title = dream_get_the_archive_title();
				// for blog page
				if( is_home() ) {
						$page_for_posts = get_option( 'page_for_posts' );
						$title = ($page_for_posts != 0) ? get_the_title($page_for_posts) : esc_html__('Blog', 'dream');
				}
			}
		}
		?>
...
if(!function_exists('dream_title_bar')) :
	function dream_title_bar() {
		if (!defined('FW')) { dream_title_bar_default(); return; }
		...
		/* variables */
		$posts_header_height = $description = $dream_overlay_style = $title =  $taxonomy = $term_id = '';
		...
		if( is_page() ){
			...
			if(isset($title_bar_overlay_options['title_bar_overlay']) && $title_bar_overlay_options['title_bar_overlay'] == 'yes') {
				$dream_overlay_bg = $title_bar_overlay_options['yes']['title_bar_overlay_color']['id'];
				$dream_opacity    = $title_bar_overlay_options['yes']['title_bar_overlay_opacity_image'] / 100;
				if ( $dream_overlay_bg == 'fw-custom' ) {
					if ( ! empty( $title_bar_overlay_options['yes']['title_bar_overlay_color']['color'] ) ) {
						$dream_overlay_style = '<div class="fw-main-row-overlay" style="background-color: ' . $title_bar_overlay_options['yes']['title_bar_overlay_color']['color'] . '; opacity: ' . $dream_opacity . ';"></div>';
					}
				} else {
					$dream_overlay_style = '<div class="fw-main-row-overlay fw_theme_bg_' . $dream_overlay_bg . '" style="opacity: ' . $dream_opacity . ';"></div>';
				}
			}
		...
		if(! empty($image) ||
			...
			<section
				...
				<?php echo "{$dream_overlay_style}"; ?>
				<div class="container" style="<?php echo esc_attr($space_top_bottom); ?>">
					<div class="row">
						<div class="col-sm-12">
							<div class="fw-heading <?php echo esc_attr($content_align); ?>">
								<h1 class="fw-special-title"><?php echo dream_translate($title); ?></h1>
								<?php if($description != '' ) : ?>
									<div class="fw-special-subtitle"><?php echo dream_translate($description); ?></div>
								<?php endif; ?>
								<div class="breadcrumbs-right"><?php if( function_exists('fw_ext_breadcrumbs') && bearsthemes_check_is_bbpress() == '' ) fw_ext_breadcrumbs('<span>-</span>'); ?></div>
							</div>
						</div>
					</div>
				</div>
...
if(!function_exists('dream_fw_ext_page_builder_is_builder_post')) :
	function dream_fw_ext_page_builder_is_builder_post($post_id = '')
	{
...
if(!function_exists('dream_get_extra_typography')) :
	/**
	 * dream_get_extra_typography
	 * @param [string] $type : element_select_option | get_style_font_by_name | build_class_css
	 * @param [string] $name
	 * @return [array]
	 */
	function dream_get_extra_typography($type = null, $name = null) {
		if ( ! defined( 'FW' ) ) return false;
		...
		$dream_color_settings = (function_exists('fw_get_db_customizer_option')) ? fw_get_db_customizer_option('color_settings') : array();
		$result = null;
		...
		switch($type) {
			case 'element_select_option' :
				if(count($customizer_option) > 0) {
					$result = array();
					/* default */
					array_push($result, array('value' => '', 'label' => esc_html__('Default', 'dream')));
				...
				if(count($customizer_option) > 0 && !empty($name)) {
					foreach($customizer_option as $index => $item) {
						...
						if($value_trim == $name_trim){
							$result = dream_get_font_array($item['typography'], $dream_color_settings);
							break;
						}
					}
				}
			...
			case 'build_class_css' :
				if(count($customizer_option) > 0) {
					$result = '';
					foreach($customizer_option as $index => $item) {
						$font_data = dream_get_font_array($item['typography'], $dream_color_settings);
						if(!empty($item['class'])) {
							$result = sprintf(
								'%s {%s}',
								$item['class'],
								dream_css_build_font_style($font_data)
							);
						}
					}
				}
...
if(!function_exists('dream_css_build_font_style')) :
	/**
	 * dream_css_build_font_style
	 * @param [array] $font_data
	 * @return [string] css
	 */
	function dream_css_build_font_style($font_data = array()) {
		$style = array();
...
if(!function_exists('dream_generate_random_key')) :
	/**
	 * dream_generate_random_key
	 * @param [int] $length
	 */
	function dream_generate_random_key($length = 10) {
	    return substr(str_shuffle(str_repeat($x='0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ', ceil($length/strlen($x)) )),1,$length);
	}
endif;

if(! function_exists('dream_product_add_to_cart_button')) :
	function dream_product_add_to_cart_button($pid = null, $data = array()) {
		$output = '';
		$product = wc_get_product( $pid );

		$text = isset($data['text']) ? $data['text'] : __('Add To Cart', 'dream');
		$option_text = isset($data['text']) ? $data['text'] : __('Select Options', 'dream');

		$button_text_default = array(
			'external' 	=> __( 'Buy product', 'dream' ),
			'grouped' 	=> __( 'View products', 'dream' ),
			'simple' 		=> __( 'Add to cart', 'dream' ),
			'variable' 	=> __( 'Select options', 'dream' ),
		);
		...
		switch ( trim($product_type) ) {
			...
			default:
				$add_to_cart_text = __( 'Read more', 'dream' );
		}
...
if(!function_exists('dream_get_products')) :
	/**
	 * WooCommerce - get Products function
	 * dream_get_products
	 * @param [array] $data
	 */
	function dream_get_products( $data = array() ) {
...
if(! function_exists('dream_gridmap_masonryhybrid_handle')) :
	/**
	 * dream_gridmap_masonryhybrid_handle
	 * @param $type [string] get | set
	 * @param $grid_name [string]
	 * @param $data [array]
	 */
	function dream_gridmap_masonryhybrid_handle($type = 'get', $grid_name = '', $data = array()) {
...
if ( ! function_exists( 'dream_related_articles' ) ) :
	/**
	 * Return post related articles
	 */
	function dream_related_articles($post_count = 2) {
		global $post;
		$taxonomy   = 'post_tag';
		$post_terms = array();
...
if(! function_exists('dream_get_header_2_options')) :
	function dream_get_header_2_options() {
		$dream_header_page_custom_settings = dream_get_options_header();
		$dream_header_settings             = defined( 'FW' ) ? $dream_header_page_custom_settings : array( 'header_type_picker' => array() );

//		$dream_header_settings = defined( 'FW' ) ? fw_get_db_customizer_option( 'header_settings' ) : array('header_type_picker' => array());
		...
		), $dream_header_settings['header_type_picker']);
		return $header_2_options;
	}
endif;

if(! function_exists('dream_load_header_2')) :
	function dream_load_header_2() {
		$header_2_options = dream_get_header_2_options();
...
if(! function_exists('dream_get_header_3_options')) :
	/**
	 * dream_get_header_3_options
	 *
	 */
	function dream_get_header_3_options() {
		$dream_header_page_custom_settings = dream_get_options_header();
		$dream_header_settings             = defined( 'FW' ) ? $dream_header_page_custom_settings : array( 'header_type_picker' => array() );
...
if(! function_exists('dream_load_logo_sidebar_header_3')) :
	/*
	 * dream_load_logo_sidebar_header_3
	 */
	function dream_load_logo_sidebar_header_3() {
			...
			<div class="bt-icell bt-icell-align-middle" style="width: <?php echo esc_attr($item['width'] . '%'); ?>">
				<div class="<?php echo esc_attr($classes); ?>">
					...
					switch ($item['type']['select']) {
						/* logo */
						case 'logo': ?>
							<div class="bt-container-logo <?php echo esc_attr($item['custom_class']); ?>"><?php dream_logo(); ?></div>
							<?php break;
...
if(! function_exists('dream_load_menu_header_3')) :
	function dream_load_menu_header_3() {
		$header_3_options = dream_get_header_3_options();
...
if(! function_exists('dream_builder_options_header')) :
	function dream_builder_options_header() {
//		$dream_header_settings = defined( 'FW' ) ? fw_get_db_customizer_option( 'header_settings' ) : array();
//		$dream_logo_settings = defined( 'FW' ) ? fw_get_db_customizer_option( 'logo_settings' ) : array();
		...
		/* Result options */
		$result = array(
			'dream_enable_header_top_bar' 	=> isset( $dream_header_settings['enable_header_top_bar']['selected_value'] ) 				? $dream_header_settings['enable_header_top_bar']['selected_value'] 					: 'no',
			'dream_enable_header_top_bar_mobi' 	=> isset( $dream_header_settings['enable_header_top_bar_mobi']['selected_value'] ) 				? $dream_header_settings['enable_header_top_bar_mobi']['selected_value'] 					: 'no',
			'dream_header_logo_align' 			=> isset( $dream_header_settings['header_type_picker']['header-1']['logo_align'] ) 		? $dream_header_settings['header_type_picker']['header-1']['logo_align'] 			: '',
			'dream_logo_retina' 						=> isset( $dream_logo_settings['logo']['image']['retina_logo'] ) 											? $dream_logo_settings['logo']['image']['retina_logo'] 												: '',
			'dream_header_full_content' 		=> isset( $dream_header_settings['enable_header_full_content'] ) 											? $dream_header_settings['enable_header_full_content'] 												: '',
			'dream_header_menu_position' 		=> isset( $dream_header_settings['header_menu_position'] ) 														? $dream_header_settings['header_menu_position'] 															: '', // fw-menu-position-right | fw-menu-position-left | fw-menu-position-center
			'dream_absolute_header' 				=> isset( $dream_header_settings['enable_absolute_header'] ) 													? $dream_header_settings['enable_absolute_header']['selected_value'] 					: '',
			'dream_sticky_header' 					=> isset( $dream_header_settings['enable_sticky_header'] ) 														? $dream_header_settings['enable_sticky_header']['selected_value'] 						: '',
			'dream_header_2_options' 				=> dream_get_header_2_options(),
			'dream_header_3_options' 				=> dream_get_header_3_options(),
		);

		/* START - Overide options use $_GET */
		$opts_overide = array('dream_enable_header_top_bar', 'dream_enable_header_top_bar_mobi', 'dream_header_full_content', 'dream_header_menu_position', 'dream_absolute_header', 'dream_sticky_header');
...
if(! function_exists('dream_get_options_portfolio')) :
function dream_get_options_portfolio() {
	// echo '<pre>'; print_r(fw_get_db_customizer_option()); echo '</pre>';
	...
	$dream_portfolio_settings = defined('FW') ? fw_get_db_customizer_option('portfolio_settings') : $default_settings;

	return $dream_portfolio_settings;
}
endif;

if(! function_exists('dream_get_category_by_id')) :
	/**
	 * dream_get_category_by_id
	 *
	 * @param [int] $product_id
	 * @return [array]
	 */
	function dream_get_category_by_id($product_id = 0) {
		$term_list = wp_get_post_terms($product_id, 'product_cat');
		return $term_list;
	}
endif;

if( ! function_exists('dream_get_all_product_cat') ) :
	function dream_get_all_product_cat($params = array()) {
		$taxonomy = 'product_cat';
...
if(! function_exists('dream_build_template_by_product_term')) :
	/**
	 * @param [array] $term_arr
	 * @param [string] $layout (classes, masonry_filter)
	 * @param [array] $params
	 */
	function dream_build_template_by_product_term($term_arr = array(), $layout = 'classes', $params = array()) {
		...
		switch ($layout) {
			...
			case 'filter_masonry':
				...
				$output .= '<a href="#" data-filter="*" class="product-filter-item is-active">'. esc_html__('All', 'dream') .'</a>';
 ...
 **/
if ( ! function_exists( 'dream_woocommerce_get_product_thumbnail' ) ) :
	function dream_woocommerce_get_product_thumbnail( $size = 'shop_catalog', $placeholder_width = 0, $placeholder_height = 0  ) {
		...
		<div class="woocommerce-imagewrapper">
			...
			<?php
			$thumbnail_html = "<img src='". get_template_directory_uri() . '/assets/images/image-default.jpg' ."' alt='". esc_html__('Thumbnail', 'dream') ."'>";
...
if(! function_exists('bearsthemes_get_gallery_grid_by_post_id')) :
	function bearsthemes_get_gallery_by_post_id($post_id, $params = array()) {
		...
		/* load grid map */
		$grid_map = dream_gridmap_masonryhybrid_handle('get', $masonryhybrid_resize_opts['grid_name']);
		if(! empty($grid_map)) {
...
if(! function_exists('dream_render_recipe_media_single')) :
  function dream_render_recipe_media_single($post_id = null, $template = null) {
    global $post;
			...
			<div class="recipe-media-content">
				<?php if(! empty($recipe_post_data['video'])) : ?>
					<div class="<?php echo esc_attr($recipe_post_data['class_item']); ?>">
						<h4 class="title"><?php echo esc_html__('Video', 'dream') ?></h4>
				...
				<?php if(count($recipe_post_data['gallery_data']) > 0) : ?>
					<div class="<?php echo esc_attr($recipe_post_data['class_item']); ?>">
						<h4 class="title"><?php echo esc_html__('Gallery', 'dream') ?></h4>
						<?php
				...
				<div class="<?php echo esc_attr($recipe_post_data['class_item']); ?>">
					<h4 class="title"><?php echo esc_html__('Sharing', 'dream') ?></h4>
					<?php echo do_shortcode('[x_share title="'. esc_html__(' ', 'dream') .'" facebook="true" twitter="true" google_plus="true" linkedin="true" pinterest="true"]'); ?>
				</div>
			</div>
		</div>
		<?php
...
if(! function_exists('dream_share_post')) :
	function dream_share_post($params = array('facebook' => false, 'twitter' => false, 'google_plus' => false, 'linkedin' => false, 'pinterest' => faslse)) {
		global $post;
		...
		/* facebook */
		if($facebook == true) $output .= '<a class="share-post-item s-facebook" href="http://www.facebook.com/sharer.php?u={post_link}" target="_blank" data-toggle="tooltip" title="'. __('Share on Facebook', 'dream') .'" data-share-post>'. $share_icon['facebook'] .'</a>';
		/* twitter */
		if($twitter == true) $output .= '<a class="share-post-item s-twitter" href="https://twitter.com/share?url={post_link}&text={post_title}" target="_blank" data-toggle="tooltip" title="'. __('Share on Twitter', 'dream') .'" data-share-post>'. $share_icon['twitter'] .'</a>';
		/* google plus */
		if($google_plus == true) $output .= '<a class="share-post-item s-google-plus" href="https://plus.google.com/share?url={post_link}" target="_blank" data-toggle="tooltip" title="'. __('Share on Google+', 'dream') .'" data-share-post>'. $share_icon['google_plus'] .'</a>';
		/* linkedin */
		if($linkedin == true) $output .= '<a class="share-post-item s-linkedin" href="http://www.linkedin.com/shareArticle?mini=true&url={post_link}" target="_blank" data-toggle="tooltip" title="'. __('Share on LinkedIn', 'dream') .'" data-share-post>'. $share_icon['linkedin'] .'</a>';
		/* pinterest */
		if($pinterest == true) $output .= '<a class="share-post-item s-pinterest" href="https://pinterest.com/pin/create/bookmarklet/?url={post_link}&description={post_title}" target="_blank" data-toggle="tooltip" title="'. __('Share on Pinterest', 'dream') .'" data-share-post>'. $share_icon['pinterest'] .'</a>';
...
if(! function_exists('dream_give_get_donor')) :
	function dream_give_get_donor($args = array()) {
...
if(! function_exists('dream_give_donors_slide')) :
	function dream_give_donors_slide($donors = array()) {
...
if(! function_exists('dream_load_icon_v2')) :
	/**
	 * dream_load_icon_v2
	 * @since 0.0.7
	 */
	function dream_load_icon_v2($data = array(), $custom_class = '') {
 ...
 * @return string
 */
if ( !function_exists( 'dream_get_feature_image' ) ) {
	function dream_get_feature_image( $attachment_id, $size_type = null, $width = null, $height = null, $alt = null, $title = null ) {
...
if ( !function_exists( 'dream_get_upcoming_events' ) ) {
	function dream_get_upcoming_events( $args = array() ) {
...
if ( !function_exists( 'dream_get_expired_events' ) ) {
	function dream_get_expired_events( $args = array() ) {
		if ( is_tax( 'tp_event_category' ) ) {
		    if( version_compare(WPEMS_VER, '2.1.5', '>=') ) {
                $args = wp_parse_args(
                    $args,
...
if ( !function_exists( 'dream_get_happening_events' ) ) {
	function dream_get_happening_events( $args = array() ) {
		if ( is_tax( 'tp_event_category' ) ) {
            if( version_compare(WPEMS_VER, '2.1.5', '>=') ) {
                $args = wp_parse_args(
                    $args,
 ...
 * Get page url
 */
if ( !function_exists('dream_get_page_url' ) ) {
	function dream_get_page_url( $page ) {
 ...
 */
if ( !function_exists( 'dream_archive_event_template' ) ) {
	function dream_archive_event_template( $template ) {
		if ( get_post_type() == 'tp_event' && is_post_type_archive( 'tp_event' ) || is_tax( 'tp_event_category' ) ) {
			$GLOBALS['dream_happening_events'] = dream_get_happening_events();
			$GLOBALS['dream_upcoming_events']  = dream_get_upcoming_events();
			$GLOBALS['dream_expired_events']   = dream_get_expired_events();
		}
...
if ( !function_exists( 'dream_add_grid_item_predefined_templates_event' ) ) {
	function dream_add_grid_item_predefined_templates_event( $templates ) {
		$templates['event_grid'] = array(
			'name' => __( 'Event grid: Default', 'js_composer' ),
			'template' => '[vc_gitem c_zone_position="bottom"][vc_gitem_animated_block][vc_gitem_zone_a height_mode="4-3" link="post_link" featured_image="yes" css=".vc_custom_1539231029140{background-color: #ffffff !important;}" el_class="event-grid-classic"][vc_gitem_row position="top"][vc_gitem_col][/vc_gitem_col][/vc_gitem_row][vc_gitem_row position="middle"][vc_gitem_col][/vc_gitem_col][/vc_gitem_row][vc_gitem_row position="bottom"][vc_gitem_col width="1/2"][vc_icon icon_fontawesome="fa fa-calendar-check-o" color="custom" background_style="rounded" background_color="custom" custom_color="#ffffff" custom_background_color="#2e91fc" el_class="event-icon-grid-classic" css=".vc_custom_1539231375478{margin-bottom: 0px !important;}"][/vc_gitem_col][vc_gitem_col width="1/2"][/vc_gitem_col][/vc_gitem_row][/vc_gitem_zone_a][vc_gitem_zone_b][vc_gitem_row position="top"][vc_gitem_col][/vc_gitem_col][/vc_gitem_row][vc_gitem_row position="middle"][vc_gitem_col width="1/2"][/vc_gitem_col][vc_gitem_col width="1/2"][/vc_gitem_col][/vc_gitem_row][vc_gitem_row position="bottom"][vc_gitem_col][/vc_gitem_col][/vc_gitem_row][/vc_gitem_zone_b][/vc_gitem_animated_block][vc_gitem_zone_c css=".vc_custom_1539241977388{padding-top: 30px !important;background-color: #ffffff !important;}" el_class="event-grid-classic-content"][vc_gitem_row][vc_gitem_col width="5/12"][vc_gitem_event source="day" el_class="event-day-grid-classic"][/vc_gitem_col][vc_gitem_col width="7/12"][vc_gitem_event source="time" el_class="event-time-grid-classic"][/vc_gitem_col][/vc_gitem_row][vc_gitem_row][vc_gitem_col css=".vc_custom_1539243499799{padding-top: 0px !important;padding-bottom: 0px !important;}"][vc_gitem_post_title link="post_link" font_container="tag:h4|text_align:left" css=".vc_custom_1539243561051{margin-top: 0px !important;margin-bottom: 0px !important;}"][vc_gitem_post_excerpt font_container="tag:p|text_align:left"][vc_gitem_event source="location" el_class="event-local-grid-classic"][/vc_gitem_col][/vc_gitem_row][/vc_gitem_zone_c][/vc_gitem]',
		);
		$templates['event_grid_1'] = array(
			'name' => __( 'Event grid: Style 1', 'js_composer' ),
			'template' => '[vc_gitem c_zone_position="bottom" el_class="event-style1"][vc_gitem_animated_block][vc_gitem_zone_a height_mode="4-3" link="post_link" featured_image="yes" css=".vc_custom_1542697265395{background-color: #ffffff !important;}" el_class="event-style1-thumb"][vc_gitem_row position="top"][vc_gitem_col][/vc_gitem_col][/vc_gitem_row][vc_gitem_row position="middle"][vc_gitem_col][/vc_gitem_col][/vc_gitem_row][vc_gitem_row position="bottom"][vc_gitem_col][/vc_gitem_col][/vc_gitem_row][/vc_gitem_zone_a][vc_gitem_zone_b][vc_gitem_row position="top"][vc_gitem_col][/vc_gitem_col][/vc_gitem_row][vc_gitem_row position="middle"][vc_gitem_col width="1/2"][/vc_gitem_col][vc_gitem_col width="1/2"][/vc_gitem_col][/vc_gitem_row][vc_gitem_row position="bottom"][vc_gitem_col][/vc_gitem_col][/vc_gitem_row][/vc_gitem_zone_b][/vc_gitem_animated_block][vc_gitem_zone_c css=".vc_custom_1542697282157{background-color: #ffffff !important;}" el_class="event-style1-content"][vc_gitem_row][vc_gitem_col css=".vc_custom_1542700055117{padding-bottom: 0px !important;}"][vc_icon icon_fontawesome="fa fa-calendar-check-o" color="custom" el_class="event-icon-style1"][vc_gitem_event source="day"][/vc_gitem_col][/vc_gitem_row][vc_gitem_row][vc_gitem_col][vc_gitem_post_title link="post_link" font_container="tag:h4|text_align:left" css=".vc_custom_1539243561051{margin-top: 0px !important;margin-bottom: 0px !important;}"][vc_gitem_event source="time" el_class="event-time-grid-classic"][vc_gitem_event source="location" el_class="event-local-grid-classic"][/vc_gitem_col][/vc_gitem_row][vc_gitem_row][vc_gitem_col el_class="event-ticket"][vc_btn link="post_link" title="BUY TICKETS" style="custom" custom_text="#666666" align="center" button_block="true" css=".vc_custom_1542699066743{margin-bottom: 0px !important;}"][/vc_gitem_col][/vc_gitem_row][/vc_gitem_zone_c][/vc_gitem]',
		);
		return $templates;
	}
}

if ( !function_exists( 'dream_add_query_vars_filter' ) ) {
	function dream_add_query_vars_filter( $vars ) {
		$vars[] = "layout";
  	return $vars;
	}
}

if ( !function_exists( 'dream_add_class_layout_body' ) ) {
	function dream_add_class_layout_body( $class ) {
		$post_types = 'lp_course';
		$layout = get_query_var('layout','grid');
		$layout = $layout ? $layout : 'grid';
		$class[] = 'archive-lp_course-'.$layout;
		...
		if ( defined( 'FW' )  ) {
			...
			// }
			if ( is_page() &&
				fw_akg( 'enable_page_options/selected_value', fw_get_db_post_option( $post->ID ) ) == 'fw_enable_page_options' &&
			 ! empty( fw_akg( 'enable_page_options/fw_enable_page_options/header_custom_class', fw_get_db_post_option( $post->ID ) ) )
		 ) {
				$class[] = fw_akg( 'enable_page_options/fw_enable_page_options/header_custom_class', fw_get_db_post_option( $post->ID ) );
			}

			if(is_page() &&
				fw_akg( 'enable_page_options/selected_value', fw_get_db_post_option( $post->ID ) ) == 'fw_enable_page_options' &&
				! empty( fw_akg( 'enable_page_options/fw_enable_page_options/custom_page_header_layout_value', fw_get_db_post_option( $post->ID ) ))
			){
				$extend_header = fw_akg( 'enable_page_options/fw_enable_page_options/custom_page_header_layout_value', fw_get_db_post_option( $post->ID ) );
				$class[] = fw_akg( 'enable_page_options/fw_enable_page_options/header_custom_class', fw_get_db_post_option( $extend_header ) );
			}
		}
		return $class;
	}
...
if ( !function_exists( 'dream_wpems_single_event_meta' ) ) {
	function dream_wpems_single_event_meta( ) {
		wpems_get_template( 'loop/meta.php' );
	}
}
if ( !function_exists( 'dream_wpems_single_event_share_tags' ) ) {
	function dream_wpems_single_event_share_tags( ) {
		wpems_get_template( 'loop/share-tags.php' );
	}
}
/**
 * Get login page url
 ...
 * @return false|string
 */
if ( ! function_exists( 'dream_get_login_page_url' ) ) {
	function dream_get_login_page_url() {

		if ( $page = get_option( 'dream_login_page' ) ) {
...
if ( ! function_exists( 'dream_checkout_link_login_register' ) ) {
	function dream_checkout_link_login_register() {
		if ( is_user_logged_in() ) {
			return;
		}
		$redirect      = ( ! empty( $_SERVER['HTTPS'] ) ? "https" : "http" ) . '://' . $_SERVER["HTTP_HOST"] . $_SERVER["REQUEST_URI"];
		$link_login    = dream_get_login_page_url() . '?redirect_to=' . esc_attr( $redirect );
		$link_register = dream_get_register_url() . '&redirect_to=' . esc_attr( $redirect );
		if ( LP()->checkout()->is_enable_login() || LP()->checkout()->is_enable_register() ) {
			...
			if ( LP()->checkout()->is_enable_login() ) {
				echo 'You can <a href="' . esc_attr( $link_login ) . '">' . __( 'login', 'dream' ) . '</a> now.';
			}
			if ( LP()->checkout()->is_enable_register() ) {
				echo 'Don\'t have an account? Click <a href="' . esc_attr( $link_register ) . '">' . __( 'register now', 'dream' ) . '</a>';
			}
			echo '</div>';
		}
	}
...
if ( ! function_exists( 'dream_get_options_header' ) ) :
	/**
	 * @return array|mixed|null
	 */
	function dream_get_options_header() {
		if ( ! defined( 'FW' ) ) {
			return;
		}
		$dream_options_header            = array();
		...
		if ( fw_akg( 'enable_page_options/selected_value', fw_get_db_post_option() ) == 'fw_enable_page_options' ) {
			if ( empty( $selected_custom_page_header_id ) ) {
				$dream_options_header = fw_akg( 'enable_page_options/fw_enable_page_options/header_settings', fw_get_db_post_option() );
			} else {
				$dream_options_header = fw_akg( 'enable_page_options/fw_enable_page_options/header_settings', fw_get_db_post_option( $selected_custom_page_header_id ) );
			}
		} else {
			$dream_options_header = fw_get_db_customizer_option( 'header_settings' );
		}

		return $dream_options_header;
	}
endif;

if ( ! function_exists( 'dream_scss_page_options_handle' ) ) :
	function dream_scss_page_options_handle( $scss_content_string = '' ) {
		$scss_content = '';
		if ( class_exists( 'scssc' ) ) {
			$scss = new scssc();
			$scss->setImportPaths( get_template_directory() . '/assets/style-scss/' );
			$scss->setFormatter( 'scss_formatter_compressed' );
			$scss_content = $scss->compile( $scss_content_string . '@' . 'import "main.scss"' );
		}
