			...
			<li>
				<?php if($instance['show_images']) : ?>
					<?php if($item['post_img'] != '') : ?>
						...
						// $image = dream_get_image( $item['post_img']['attachment_id'], $args );
						?>
					...
					<?php else : ?>
						<div class="fw-widget-post-image fw-block-image-parent fw-overlay-1 fw-image-frame">
							<a href="<?php echo esc_url($item['post_link']); ?>" class="fw-thumbnail-post-list">
								<img data-src="<?php echo esc_url(get_template_directory_uri()).'/images/no-photo-max-size.jpg'; ?>" src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==" class="lazyload" alt="<?php esc_html_e('Lazyload', 'dream') ?>" />
							</a>
						</div>
					<?php endif; ?>
				<?php endif; ?>
