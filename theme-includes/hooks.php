<?php
if ( ! function_exists( '_dream_action_theme_setup' ) ) :
	/**
	 * Theme setup.
	 ...
	 * @internal
	 */
	function _dream_action_theme_setup() {
		...
		add_image_size( 'dream-image-large', 1228, 691, true );
		add_image_size( 'dream-image-medium', 614, 346, true );
		add_image_size( 'dream-image-small', 295, 166, true );

		add_image_size( 'dream-image-square-800', 800, 800, true ); // 1:1
		add_image_size( 'dream-image-square-300', 300, 300, true ); // 1:1
		...
		add_theme_support( 'featured-content', array(
			'featured_content_filter' => 'dream_get_featured_posts',
			'max_posts'               => 6,
		) );
...
add_action( 'after_setup_theme', '_dream_action_theme_setup' );

if ( ! function_exists( '_dream_action_theme_includes_additional_option_types' ) ) :
	/**
	 * Include addition option types
	 */
	function _dream_action_theme_includes_additional_option_types() {
		$dream_template_directory = get_template_directory();

		load_template( $dream_template_directory . '/theme-includes/includes/option-types/color-palette/class-fw-color-palette-new.php' );
		load_template( $dream_template_directory . '/theme-includes/includes/option-types/tf-animation/class-fw-option-type-tf-animation.php' );
		load_template( $dream_template_directory . '/theme-includes/includes/option-types/tf-typography/class-fw-option-type-tf-typography.php' );
	}

	add_action( 'fw_option_types_init', '_dream_action_theme_includes_additional_option_types' );
endif;

function _dream_init() {
	...
	// check Vc exist
	if ( class_exists( 'Vc_Manager' ) ) : _dream_js_composer_init_hook(); endif;

	// add_filter( 'http_request_args', '_bearsthemes_deny_theme_updates', 5, 2 );

	if ( class_exists( 'Give' ) ) : _dream_give_init_hook(); endif;
...

{
	 ...
	 */
	if ( ! function_exists( '_dream_give_init_hook' ) ) :
		function _dream_give_init_hook() {
			/* single */
			add_filter( 'give_default_wrapper_start', '_dream_give_default_wrapper_start' );
			add_filter( 'give_default_wrapper_end', '_dream_give_default_wrapper_end' );

			add_filter( 'give_left_sidebar_pre_wrap', '_dream_give_left_sidebar_pre_wrap' );
			add_filter( 'give_left_sidebar_post_wrap', '_dream_give_left_sidebar_post_wrap' );

			add_action( 'give_before_single_form_summary', '_dream_give_before_single_form_summary', 2 );
			add_action( 'give_after_single_form_summary', '_dream_give_load_sidebar', 10 );

			add_action( 'pre_get_posts', '_dream_give_set_posts_per_page', 10 );

			/* _dream_action_give_single_form_summary */
			// add_action( '_dream_action_give_single_form_summary', '_dream_give_get_donation_form', 10 );
			add_action( '_dream_action_give_single_form_summary', 'give_get_donation_form', 10 );
		}
	endif;

	if ( ! function_exists( '_dream_give_get_donation_form' ) ) :
		/**
		 * Get Donation Form.
		 * @since  1.0
		 *
		 ...
		 */
		function _dream_give_get_donation_form( $args = array() ) {
			...
			if ( ( 'publish' !== $form->post_status && ! current_user_can( 'edit_give_forms', $form->ID ) ) || ( 'trash' === $form->post_status ) ) {
				return false;
			}
            ...
            <div id="give-form-<?php echo esc_attr( $form->ID ); ?>-wrap" class="<?php echo esc_attr( $form_wrap_classes ); ?>">
				...
				<?php if ( $form->is_close_donation_form() ) {
					...
					if ( ( isset( $args['show_title'] ) && $args['show_title'] == true ) && ! doing_action( 'give_single_form_summary' ) ) {
						echo wp_kses( $form_title, array( 'h2' => array( 'class' => array() ) ) );
					}
                    ...
                    <form id="give-form-<?php echo esc_attr( $form_id ); ?>" class="<?php echo esc_attr( $form_classes ); ?>"
	...
	if ( ! function_exists( '_dream_give_set_posts_per_page' ) ) :
		function _dream_give_set_posts_per_page( $query ) {
			$_FW = defined( 'FW' );
			if ( $_FW ) {
	...
	if ( ! function_exists( '_dream_give_before_single_form_summary' ) ) :
		function _dream_give_before_single_form_summary() {
			remove_action( 'give_before_single_form_summary', 'give_get_forms_sidebar', 20 );
		}
	endif;

	/**
	 * _dream_give_load_sidebar
	 */
	if ( ! function_exists( '_dream_give_load_sidebar' ) ) :
		function _dream_give_load_sidebar() {
	...
	/**
	 * _dream_give_default_wrapper_start
	 */
	if ( ! function_exists( '_dream_give_default_wrapper_start' ) ) :
		function _dream_give_default_wrapper_start( $output ) {
			return '';
		}
	endif;
	 ...
	 * _dream_give_default_wrapper_start
	 */
	if ( ! function_exists( '_dream_give_default_wrapper_end' ) ) :
		function _dream_give_default_wrapper_end( $output ) {
			return '';
		}
	endif;

	/**
	 * _dream_give_left_sidebar_pre_wrap
	 */
	if ( ! function_exists( '_dream_give_left_sidebar_pre_wrap' ) ) {
		function _dream_give_left_sidebar_pre_wrap( $output ) {
			return '<div id="single-give-thubnail" class="single-give-thubnail">';
		}
	}

	/**
	 * _dream_give_left_sidebar_post_wrap
	 */
	if ( ! function_exists( '_dream_give_left_sidebar_post_wrap' ) ) {
		function _dream_give_left_sidebar_post_wrap( $output ) {
			return '</div>';
		}
	}
}
...
if ( ! function_exists( '_bearsthemes_filter_fdm_styles' ) ) :
	function _bearsthemes_filter_fdm_styles( $style ) {
		// print_r($style); die;
		$style['custom'] = new fdmStyle( array(
				'id'    => 'custom_theme',
				'label' => __( 'Custom Style (themes)', 'dream' ),
				'css'   => array(
					'base' => FDM_PLUGIN_URL . '/assets/css/base.css',
				)
			) );

		$style['custom_2'] = new fdmStyle( array(
				'id'    => 'custom_thumb_round_theme',
				'label' => __( 'Custom Thumb Round Style (themes)', 'dream' ),
				'css'   => array(
					'base' => FDM_PLUGIN_URL . '/assets/css/base.css',
				)
			) );
...
if ( ! function_exists( '_dream_js_composer_init_hook' ) ) :
	function _dream_js_composer_init_hook() {
...
if ( ! function_exists( '_dream_google_fonts_url' ) ) :
	/**
	 * Register Google fonts.
	 * @return string Google fonts URL for the theme.
	 */
	function _dream_google_fonts_url() {
		$fonts_url = '';
		$fonts     = array();
		$subsets   = 'latin,latin-ext';

		/* translators: If there are characters in your language that are not supported by this font, translate this to 'off'. Do not translate into your own language. */
		if ( 'off' !== esc_html_x( 'on', 'Raleway font: on or off', 'dream' ) ) {
...
if ( ! function_exists( '_dream_admin_enqueue_scripts' ) ) :
	function _dream_admin_enqueue_scripts() {
		wp_enqueue_style( 'dream-google-font-backend', _dream_google_fonts_url(), false, '1.0.0' );
	}
endif;
add_action( 'admin_enqueue_scripts', '_dream_admin_enqueue_scripts' );

if ( ! function_exists( '_dream_woocommerce_init_hook' ) ) :
	function _dream_woocommerce_init_hook() {
		// woo 3.x
		...
		add_action( 'woocommerce_before_shop_loop_item_title', '_dream_woocommerce_template_loop_product_thumbnail', 10 );
		...
		if ( function_exists( 'YITH_WCQV_Frontend' ) ) :
			...
			add_action( 'bearsthemes_woocommerce_after_thumbnail_loop', array(
				YITH_WCQV_Frontend(),
				'yith_add_quick_view_button'
			), 10 );
		endif;
...
add_action( 'init', '_dream_woocommerce_init_hook' );
 ...
 **/
if ( ! function_exists( '_dream_woocommerce_template_loop_product_thumbnail' ) ) {
	function _dream_woocommerce_template_loop_product_thumbnail() {
		echo dream_woocommerce_get_product_thumbnail();
	}
}

if ( ! function_exists( '_dream_action_widgets_init' ) ) :
	/**
	 * Register widget areas
	 * @internal
	 */
	function _dream_action_widgets_init() {
		$beforeWidget = '<aside id="%1$s" class="widget %2$s">';
		$afterWidget  = '</aside>';
		$beforeTitle  = '<h2 class="widget-title"><span>';
		$afterTitle   = '</span></h2>';
		register_sidebar( array(
			'name'          => esc_html__( 'General Widget Area', 'dream' ),
			'id'            => 'sidebar-1',
			'description'   => '',
		...
		register_sidebar( array(
			'name'          => esc_html__( 'Events Widget Area', 'dream' ),
			'id'            => 'sidebar-events',
			'description'   => '',
			'before_widget' => $beforeWidget,
			'after_widget'  => $afterWidget,
...
add_action( 'widgets_init', '_dream_action_widgets_init' );

if ( ! function_exists( '_dream_scroll_to_top_button' ) ) :
	function _dream_scroll_to_top_button() {
		$enable_scroll_to = ( function_exists( 'fw_get_db_customizer_option' ) ) ? fw_get_db_customizer_option( 'enable_scroll_to' ) : 'no';
		if ( $enable_scroll_to == 'yes' ) {
			echo '<a id="scroll-to-top-button" href="#" title="' . __( 'Back to top', 'dream' ) . '"><span></span></a>';
		}
	}
endif;
add_action( 'wp_footer', '_dream_scroll_to_top_button', 20 );

if ( ! function_exists( '_dream_add_extra_meta_tag' ) ) :
	function _dream_add_extra_meta_tag() {
		global $dream_color_settings;
		$theme_color = ( is_array( $dream_color_settings ) && isset( $dream_color_settings['color_1'] ) ) ? $dream_color_settings['color_1'] : '#333';
		?>
	...
	add_action( 'wp_head', '_dream_add_extra_meta_tag' );
endif;

if ( ! function_exists( '_dream_action_theme_generate_styles' ) ) :
	/**
	 * Scss handle
	 * Mode Developer TRUE: SCSS always running
	 * Mode Developer FALSE: SCSS run when admin change theme settings
	 */
	function _dream_action_theme_generate_styles() {
		global $dream_scss_variables;
		$dream_scss_variables = array();
		$quick_css                = '';
		$scss_content             = '';
		$scss_extra_string        = '';
		$last_md5_scss            = get_option( 'dream_less_option_hash', '' );

		if ( defined( 'FW' ) ) {

			fw_render_view( get_template_directory() . '/theme-includes/styling.php', array(), false );
			$dream_scss_content = dream_scss_variables_handle( $dream_scss_variables );


			if ( function_exists( 'fw_get_db_settings_option' ) ) {
				$quick_css         = fw_get_db_settings_option( 'quick_css' );
				$scss_extra_string .= str_replace( array(
					'{accent-color}',
					'{secondary-color}'
				), array( '$theme-color-1', '$theme-color-2' ), $quick_css );
			}

			/* extra font */
			$scss_extra_string .= dream_get_extra_typography( 'build_class_css' );

			if ( WP_DEBUG ) {
				/**
				 * On debug scss always running
				 */
				$scss_content = implode( '; ', $dream_scss_content );
				dream_scss_handle( $scss_content . ';' . $scss_extra_string );
			} else {
				$md5_scss_variables = md5( serialize( $dream_scss_content ) );

				if ( $last_md5_scss != $md5_scss_variables ) {

					update_option( 'dream_less_option_hash', $md5_scss_variables );
					$scss_content = implode( '; ', $dream_scss_content );
					// dream_scss_handle($scss_content);
					dream_scss_handle( $scss_content . ';' . $scss_extra_string );
				}
			}
		}
...
add_action( 'wp_enqueue_scripts', '_dream_action_theme_generate_styles', 99 );

if ( ! function_exists( '_dream_action_print_google_fonts_link' ) ) :
	/**
	 * Print google fonts link
	 */
	function _dream_action_print_google_fonts_link() {
		...
		if ( ! is_singular() && function_exists( 'get_term_meta' ) ) {
			...
			if ( isset( $term->term_id ) ) {
				...
				if ( ! empty( $fw_theme_post_google_fonts ) ) {
					$fw_theme_google_fonts_list = dream_array_merge_recursive( $fw_theme_google_fonts_list, $fw_theme_post_google_fonts );
				}
			}
		} else {
			if ( isset( $post->ID ) ) {
		...
		if ( ! empty( $fw_theme_google_fonts_list ) ) {
			wp_register_style( 'fw-googleFonts', dream_get_remote_fonts( $fw_theme_google_fonts_list ) );
			wp_enqueue_style( 'fw-googleFonts' );
		}
	}
endif;
add_action( 'wp_print_styles', '_dream_action_print_google_fonts_link' );

if ( ! function_exists( '_dream_action_set_global_colors' ) ) :
	/**
	 * Set global colors
	 */
	function _dream_action_set_global_colors() {
		global $dream_color_settings;
		$dream_colors         = array(
			'color_1' => '#88C000',
			'color_2' => '#ACF300',
		...
		$dream_color_settings = function_exists( 'fw_get_db_customizer_option' ) ? fw_get_db_customizer_option( 'color_settings', $dream_colors ) : $dream_colors;
	}
endif;
add_action( 'init', '_dream_action_set_global_colors' );

if ( ! function_exists( '_dream_action_dashboard_palette_colors' ) ) :
	/**
	 * Dashboard color palette styles
	 */
	function _dream_action_dashboard_palette_colors() {
		global $dream_color_settings;
		echo '<style>
			.fw_theme_bg_color_1{ background-color: ' . $dream_color_settings['color_1'] . '; }
			.fw_theme_bg_color_2{ background-color: ' . $dream_color_settings['color_2'] . '; }
			.fw_theme_bg_color_3{ background-color: ' . $dream_color_settings['color_3'] . '; }
			.fw_theme_bg_color_4{ background-color: ' . $dream_color_settings['color_4'] . '; }
			.fw_theme_bg_color_5{ background-color: ' . $dream_color_settings['color_5'] . '; }
		</style>';
	}
endif;
add_action( 'admin_head', '_dream_action_dashboard_palette_colors' );

if ( ! function_exists( '_dream_action_count_post_visits' ) ) :
	/**
	 * Count post visits
	 */
	function _dream_action_count_post_visits() {
		if ( ! is_single() ) {
			return;
		}
		global $post;
...
add_action( 'wp_head', '_dream_action_count_post_visits' );

if ( ! function_exists( '_dream_filter_body_class' ) ) :
	/**
	 * Filter for add space/top-bar/absolute-header class
	 *
	 * @param array $classes
	 */
	function _dream_filter_body_class( $classes ) {
		global $coming_soon;

		if ( function_exists( 'fw_get_db_settings_option' ) ) {
			$dream_general_settings = defined( 'FW' ) ? fw_get_db_customizer_option() : array();

			if ( isset( $dream_general_settings['container_site_type']['selected'] ) ) {
				if ( ! $coming_soon ) {
					if ( isset( $_GET['container_site_type'] ) && ! empty( $_GET['container_site_type'] ) ) {
						$classes[] = $_GET['container_site_type'];
					} else {
						$classes[] = $dream_general_settings['container_site_type']['selected'];
					}
				}
			}
	...
	add_filter( 'body_class', '_dream_filter_body_class' );
endif;

if ( ! function_exists( '_dream_action_theme_includes_tgm' ) ) :
	/**
	 * Include TGM
	 */
	function _dream_action_theme_includes_tgm() {
		$theme_id          = defined( 'FW' ) ? fw()->theme->manifest->get_id() : 'dream';
	...
	add_action( 'after_setup_theme', '_dream_action_theme_includes_tgm' );
endif;

if ( ! function_exists( '_dream_action_shortcode_section_enqueue_dynamic_css' ) ):
	/**
	 * @internal
	 *
	 * @param array $data
	 */
	function _dream_action_shortcode_section_enqueue_dynamic_css( $data ) {
		$shortcode = 'section';
	...
	add_action( 'fw_ext_shortcodes_enqueue_static:section', '_dream_action_shortcode_section_enqueue_dynamic_css' );
endif;

if ( ! function_exists( '_dream_action_shortcode_column_enqueue_dynamic_css' ) ):
	/**
	 * @internal
	 *
	 * @param array $data
	 */
	function _dream_action_shortcode_column_enqueue_dynamic_css( $data ) {
		$shortcode = 'column';
		$atts      = shortcode_parse_atts( $data['atts_string'] );
		...
		if ( isset( $atts['background_options']['background'] ) && $atts['background_options']['background'] == 'image' && ! empty( $atts['background_options']['image']['background_image']['data'] ) ) {
			...
			$bg = dream_get_color_palette_color_and_class( $atts['background_options']['image']['background_color'], array( 'return_color' => true ) );
			if ( ! empty( $bg['color'] ) ) {
			...
			if ( $overlay == 'yes' ) {
				$dream_opacity_param = 'overlay_opacity_' . $atts['background_options']['background'];
				$dream_opacity       = $atts['background_options'][ $type ]['overlay_options']['yes'][ $dream_opacity_param ] / 100;

				$overlay_color = dream_get_color_palette_color_and_class( $atts['background_options'][ $type ]['overlay_options']['yes']['background'], array( 'return_color' => true ) );
				if ( ! empty( $overlay_color['color'] ) ) {
					$final_styles .= '.tf-sh-' . $atts['unique_id'] . ' .fw-main-row-overlay {background-color: ' . $overlay_color['color'] . '; opacity: ' . $dream_opacity . ';}';
				}
			}
		} elseif ( isset( $atts['background_options']['background'] ) && $atts['background_options']['background'] == 'color' ) {
			$bg = dream_get_color_palette_color_and_class( $atts['background_options']['color']['background_color'], array( 'return_color' => true ) );
			if ( ! empty( $bg['color'] ) ) {
	...
	add_action( 'fw_ext_shortcodes_enqueue_static:column', '_dream_action_shortcode_column_enqueue_dynamic_css' );
endif;

if ( ! function_exists( '_dream_action_shortcode_text_block_enqueue_dynamic_css' ) ):
	/**
	 * @internal
	 *
	 * @param array $data
	 */
	function _dream_action_shortcode_text_block_enqueue_dynamic_css( $data ) {
		...
		if ( isset( $atts['text_advanced_styling'] ) ) {
			...
			$text_advanced_styling = dream_get_shortcode_advanced_styles( $atts['text_advanced_styling']['text'], array( 'return_color' => true ) );
			if ( ! empty( $text_advanced_styling['styles'] ) ) {
				$final_styles .= '.tf-sh-' . $atts['unique_id'] . ' .fw-text-inner {' . $text_advanced_styling['styles'] . '}';
			}
			// responsive text styling
			$responsive_styles = dream_responsive_heading_styles( array(
				'styles'   => $atts['text_advanced_styling']['text'],
				'selector' => '.tf-sh-' . $atts['unique_id'] . ' .fw-text-inner'
			) );
			if ( ! empty( $responsive_styles ) ) {
				$final_styles .= '@media(max-width:767px){' . $responsive_styles . '}';
			}
		}
	...
	add_action( 'fw_ext_shortcodes_enqueue_static:text_block', '_dream_action_shortcode_text_block_enqueue_dynamic_css' );
endif;

if ( ! function_exists( '_dream_action_shortcode_button_enqueue_dynamic_css' ) ) :
	/**
	 * @internal
	 *
	 * @param array $data
	 */
	function _dream_action_shortcode_button_enqueue_dynamic_css( $data ) {
		$shortcode = 'button';
		$atts      = shortcode_parse_atts( $data['atts_string'] );
		...
		if ( isset( $atts['style']['selected'] ) ) {
			if ( $atts['style']['selected'] == 'fw-btn-3' ) {
				...
				if ( isset( $atts['normal_color'] ) ) {
					$normal_color = dream_get_color_palette_color_and_class( $atts['normal_color'], array( 'return_color' => true ) );
					if ( ! empty( $normal_color['color'] ) ) {
						$final_styles .= '.tf-sh-' . $atts['unique_id'] . ',.tf-sh-' . $atts['unique_id'] . ':focus{ border-bottom-color: ' . $normal_color['color'] . '; border-top-color: ' . $normal_color['color'] . ' }';
					}
				}
				// hover color
				if ( isset( $atts['hover_color'] ) ) {
					$hover_color = dream_get_color_palette_color_and_class( $atts['hover_color'], array( 'return_color' => true ) );
					if ( ! empty( $hover_color['color'] ) ) {
						$final_styles .= '.tf-sh-' . $atts['unique_id'] . ':hover { background-color: ' . $hover_color['color'] . '; border-bottom-color: ' . $hover_color['color'] . '; border-top-color: ' . $hover_color['color'] . ' }';
					}
				}
				...
				if ( isset( $atts['normal_color'] ) ) {
					$normal_color = dream_get_color_palette_color_and_class( $atts['normal_color'], array( 'return_color' => true ) );
					if ( ! empty( $normal_color['color'] ) ) {
						$final_styles .= '.tf-sh-' . $atts['unique_id'] . ',.tf-sh-' . $atts['unique_id'] . ':focus{ border-color: ' . $normal_color['color'] . ' }';
					}
				}
				// hover color
				if ( isset( $atts['hover_color'] ) ) {
					$hover_color = dream_get_color_palette_color_and_class( $atts['hover_color'], array( 'return_color' => true ) );
					if ( ! empty( $hover_color['color'] ) ) {
						$final_styles .= '.tf-sh-' . $atts['unique_id'] . ':hover { background-color: ' . $hover_color['color'] . '; border-color: ' . $hover_color['color'] . ' }';
					}
				}
				...
				if ( isset( $atts['normal_color'] ) ) {
					$normal_color = dream_get_color_palette_color_and_class( $atts['normal_color'], array( 'return_color' => true ) );
					if ( ! empty( $normal_color['color'] ) ) {
						$final_styles .= '.tf-sh-' . $atts['unique_id'] . ', .tf-sh-' . $atts['unique_id'] . ':focus{ background-color: ' . $normal_color['color'] . ' }';
					}
				}
				// hover color
				if ( isset( $atts['hover_color'] ) ) {
					$hover_color = dream_get_color_palette_color_and_class( $atts['hover_color'], array( 'return_color' => true ) );
					if ( ! empty( $hover_color['color'] ) ) {
						$final_styles .= '.tf-sh-' . $atts['unique_id'] . ':hover { background-color: ' . $hover_color['color'] . ' }';
					}
				}
		...
		if ( isset( $atts['label_advanced_styling'] ) ) {
			...
			$text_advanced_styling = dream_get_shortcode_advanced_styles( $atts['label_advanced_styling']['text'], array( 'return_color' => true ) );
			if ( ! empty( $text_advanced_styling['styles'] ) ) {
				$final_styles .= '.tf-sh-' . $atts['unique_id'] . ', .tf-sh-' . $atts['unique_id'] . ':focus {' . $text_advanced_styling['styles'] . '}';
			}
			// responsive title styling
			$title_responsive_styles = dream_responsive_heading_styles( array(
				'styles'   => $atts['label_advanced_styling']['text'],
				'selector' => '.tf-sh-' . $atts['unique_id']
			) );
			if ( ! empty( $title_responsive_styles ) ) {
				$final_styles .= '@media(max-width:767px){' . $title_responsive_styles . '}';
			}

			// hover text color
			if ( isset( $atts['label_advanced_styling']['hover_text_color'] ) ) {
				$hover_text_color = dream_get_color_palette_color_and_class( $atts['label_advanced_styling']['hover_text_color'], array( 'return_color' => true ) );
				if ( ! empty( $hover_text_color['color'] ) ) {
					$final_styles .= '.tf-sh-' . $atts['unique_id'] . ':hover { color: ' . $hover_text_color['color'] . ' }';
				}
			}
		}
	...
	add_action( 'fw_ext_shortcodes_enqueue_static:button', '_dream_action_shortcode_button_enqueue_dynamic_css' );

endif;

if ( ! function_exists( '_dream_filter_show_404_page' ) ):
	/**
	 * Show custom 404 page
	 */
	function _dream_filter_show_404_page( $template404 ) {
	...
	add_filter( '404_template', '_dream_filter_show_404_page' );
endif;

if ( ! function_exists( '_dream_action_coming_soon_page' ) ) :
	/**
	 * Coming soon page
	 */
	function _dream_action_coming_soon_page() {
		global $coming_soon;
		$coming_soon        = false;
...
add_action( 'send_headers', '_dream_action_coming_soon_page', 12 );

if ( ! function_exists( '_dream_filter_fw_settings_form_header_buttons' ) ) :
	/**
	 * Add an extra options for post event
	 */
	function _dream_filter_fw_settings_form_header_buttons( $arr ) {
		$arr2[] = '<a class="fw-theme-docs-link" target="_blank" href="#">' . esc_html__( 'Go to Docs', 'dream' ) . '</a>';
		$arr    = array_merge( $arr2, $arr );

		return $arr;
	}
endif;
add_filter( 'fw_settings_form_header_buttons', '_dream_filter_fw_settings_form_header_buttons' );

if ( ! function_exists( '_dream_filter_fw_ext_backups_db_restore_keep_options' ) ) :
	/**
	 * Add an extra options for post event
	 */
	function _dream_filter_fw_ext_backups_db_restore_keep_options( $options, $is_full ) {
		if ( ! $is_full ) {
			$options['tfuse_dream_auto_install_state'] = true;
		}

		return $options;
	}
endif;
add_filter( 'fw_ext_backups_db_restore_keep_options', '_dream_filter_fw_ext_backups_db_restore_keep_options', 10, 2 );

if ( ! function_exists( '_dream_filter_update_footer' ) ):
	/**
	 * Theme changelog in footer admin
	 */
	function _dream_filter_update_footer( $html ) {
		$dream_version = ( defined( 'FW' ) && function_exists( 'fw' ) ) ? fw()->theme->manifest->get_version() : '1.0';

		$theme_id = ( defined( 'FW' ) && function_exists( 'fw' ) ) ? fw()->theme->manifest->get_id() : 'dream';

		$html .= ' | <a href="#" target="_blank">' . wp_get_theme() . ' ' . $dream_version . '</a>';

		return $html;
	}
endif;
add_filter( 'update_footer', '_dream_filter_update_footer', 12 );

if ( ! function_exists( '_dream_custom_excerpt_length' ) ) :
	function _dream_custom_excerpt_length( $length ) {
		return 20;
	}
endif;
add_filter( 'excerpt_length', '_dream_custom_excerpt_length', 999 );

// add more link to excerpt
if ( ! function_exists( '_dream_custom_excerpt_more' ) ) :
	function _dream_custom_excerpt_more( $more ) {
		global $post;

		return '...';
	}
endif;
add_filter( 'excerpt_more', '_dream_custom_excerpt_more' );

if ( ! function_exists( '_dream_customizer_live_preview' ) ) :
	/**
	 * _dream_customizer_live_preview
	 * Fix cache file stylesheet mode Customizer Live Preview
	 */
	function _dream_customizer_live_preview() {
...
add_action( 'customize_preview_init', '_dream_customizer_live_preview' );

if ( ! function_exists( '_dreamwoocommerce_header_add_to_cart_fragment' ) ) :
	/**
	 * _dreamwoocommerce_header_add_to_cart_fragment
	 */
	function _dreamwoocommerce_header_add_to_cart_fragment( $fragments ) {
		global $woocommerce;
...
add_filter( 'add_to_cart_fragments', '_dreamwoocommerce_header_add_to_cart_fragment' );

if ( ! function_exists( '_dream_related_products_args' ) ) :
	/**
	 * _dream_related_products_args
	 */
	function _dream_related_products_args( $args ) {
		$args['posts_per_page'] = 3; // 4 related products

		return $args;
	}
endif;
add_filter( 'woocommerce_output_related_products_args', '_dream_related_products_args' );

if ( ! function_exists( '_dream_save_gridmap_masonryhybrid' ) ) :
	function _dream_save_gridmap_masonryhybrid() {
		extract( $_POST );
		dream_gridmap_masonryhybrid_handle( 'set', $params['gridname'], $params['gridmap'] );
		exit();
	}
endif;
add_action( 'wp_ajax__dream_save_gridmap_masonryhybrid', '_dream_save_gridmap_masonryhybrid' );
add_action( 'wp_ajax_nopriv__dream_save_gridmap_masonryhybrid', '_dream_save_gridmap_masonryhybrid' );
...
if ( ! function_exists( '_the_bears_filter_fw_ext_backups_demos' ) ) :
	 ...
	 */
	function _dream_filter_fw_ext_backups_demos( $demos ) {
		$demos_array = array(
			'dream' => array(
				'title'        => esc_html__( 'Dream', 'dream' ),
				'screenshot'   => 'http://package.bearsthemespremium.com/dream/dream-main/screenshot.png',
				'preview_link' => 'https://bearsthemespremium.com/theme/dream/',
			),
		);

		foreach ( $demos_array as $id => $data ) {
			$demo = new FW_Ext_Backups_Demo( $id, 'piecemeal', array(
				'url'     => 'http://package.bearsthemespremium.com/dream/',
				'file_id' => $id,
			) );
	...
	add_filter( 'fw:ext:backups-demo:demos', '_dream_filter_fw_ext_backups_demos' );
endif;
...
if ( ! function_exists( '_bearsthemes_wpurp_custom_template' ) ) :
	function _bearsthemes_wpurp_custom_template( $content, $recipe ) {
		$WPURP_Template_Recipe_Params = array(
			'max_width'     => '100%',
			'max_height'    => 'none',
			'desktop'       => '',
			...
			<div class="row">
				<div class="col-md-4">
					<div class="recipe-ingredients-wrap">
						<h4 class="recipe-title"><?php echo esc_html__( 'Ingredients', 'dream' ); ?></h4>
						<?php
				...
				<div class="col-md-8">
                    ...
                    <div class="recipe-instructions-wrap">
						<h4 class="recipe-title"><?php echo esc_html__( 'Instructions', 'dream' ); ?></h4>
						<?php
					...
					<div class="recipe-notes-wrap">
						<h4 class="recipe-title"><?php echo esc_html__( 'Notes', 'dream' ); ?></h4>
						<?php
...
if ( ! function_exists( '_bearsthemes_change_excerpt_more' ) ) :
	function _bearsthemes_change_excerpt_more( $more ) {
		global $post;
		//return ' <a href="'. get_permalink($post->ID) . '">' . __('more', 'dream') . '</a>';
	}
endif;
add_filter( 'excerpt_more', '_bearsthemes_change_excerpt_more' );

if ( ! function_exists( '_dream_admin_notice_page_builder_conflict' ) ) :
	function _dream_admin_notice_page_builder_conflict() {
		...
		if ( $cornerstone && $visual_composer ) :
            ...
            <div class="notice notice-warning is-dismissible theme-custom-notice-ui">
					...
					<div class="entry-content">
						<p><strong><?php _e( 'Page Builder Conflict !!!', 'dream' ) ?></strong></p>
						<p><?php _e( 'We found that you used two page buider plugins at the same time (Cornerstone & Visual Composer). We\'d recommend you use only one of them. Your site will be fine if you use only one of those plugins.', 'dream' ); ?></p>
					</div>
			</div>
		<?php
		endif;
...
add_action( 'admin_notices', '_dream_admin_notice_page_builder_conflict' );

if ( ! function_exists( '_dream_admin_notice_theme_message' ) ) :
	function _dream_admin_notice_theme_message() {
		$_fw = defined( 'FW' );
        ...
        <div class="notice notice-info is-dismissible theme-custom-notice-ui">
				<img style="width: 80px; margin-top: 10px;" src="<?php echo get_template_directory_uri() . '/assets/images/bears-message-icon.png'; ?>" alt="#" />
				<div class="entry-content">
					<p><strong><?php _e( sprintf( 'Hey! Thanks for your used %s theme. (current version %s)', $themename, $themeversion ), 'dream' ) ?></strong></p>
...
add_action( 'admin_notices', '_dream_admin_notice_theme_message' );

if ( ! function_exists( '_dream_notification_center_action' ) ) :
	/**
	 * _dream_notification_center_action
	 * @since 0.0.7
	 */
	function _dream_notification_center_action() {
		$_FW = defined( 'FW' );
		if ( ! $_FW ) {
			return;
		}
...
add_action( 'wp_footer', '_dream_notification_center_action' );
...
/* Hook event archive */
add_action( 'template_include', 'dream_archive_event_template' );
/* Hook fix conflic with cache plugin */
add_filter( 'vc_grid_get_grid_data_access', '__return_true' );
/* Hook add template for grid builder */
add_filter( 'vc_grid_item_predefined_templates', 'dream_add_grid_item_predefined_templates' );
/* Hook add template for grid builder */
add_filter( 'vc_grid_item_predefined_templates', 'dream_add_grid_item_predefined_templates_event' );
...
add_action( 'tp_event_single_event_content', 'dream_wpems_single_event_meta', 5 );
add_action( 'tp_event_single_event_content', 'wpems_single_event_register', 7 );
add_action( 'tp_event_loop_event_location', 'dream_wpems_single_event_share_tags', 20 );
/* Hook profile */
add_action( 'learn_press_request_handler_lp_profile_class', 'dream_update_user_profile_lp_profile_class', 10, 2 );
add_action( 'learn_press_request_handler_lp_profile_facebook', 'dream_update_user_profile_lp_profile_facebook', 10, 2 );
add_action( 'learn_press_request_handler_lp_profile_googleplus', 'dream_update_user_profile_lp_profile_googleplus', 10, 2 );
add_action( 'learn_press_request_handler_lp_profile_twitter', 'dream_update_user_profile_lp_profile_twitter', 10, 2 );
add_action( 'learn-press/end-profile-basic-information-fields', 'dream_learn_press_extra_profile_basic_information_fields', 10 );
/* new user element */
add_action( 'vc_before_init', 'dream_vc_before_init_actions' );


//Custom Page Header

if ( ! function_exists( '_dream_action_save_style_page_options' ) ) :
	function _dream_action_save_style_page_options( $post_id ) {
		if ( get_post_type() != 'page' ) {
			return;
		}
		if ( defined( 'FW' ) && class_exists( 'scssc' ) && fw_akg( 'enable_page_options/selected_value', fw_get_db_post_option() ) == 'fw_enable_page_options' ) {
			global $dream_scss_variables;
			$dream_scss_variables = array();
			$quick_css           = '';
			$scss_content        = '';
			...
			$last_md5_scss = fw_get_db_post_option( '', 'dream_scss_page_option_hash' );
			fw_render_view( get_template_directory() . '/theme-includes/styling.php', array(), false );
			$dream_scss_content = dream_scss_variables_handle( $dream_scss_variables );
			...
			if ( ( $last_md5_scss != $md5_scss_variables ) ) {
				fw_set_db_post_option( '', 'dream_scss_page_option_hash', $md5_scss_variables );
				$scss_content     = implode( '; ', $dream_scss_content );
				$scss_page_handle = dream_scss_page_options_handle( $scss_content . ';' . $scss_extra_string );
				fw_set_db_post_option( '', 'dream_custom_page_scss', $scss_page_handle );
			}
		}
	}
endif;
add_action( 'save_post', '_dream_action_save_style_page_options', 10 );

if ( ! function_exists( '_dream_action_theme_wp_print_styles' ) ) :
	function _dream_action_theme_wp_print_styles() {
		if ( defined( 'FW' ) && class_exists( 'scssc' ) ) {
			global $post;
			if ( ! $post || $post->post_type != 'page' ) {
				return;
			}
			if ( is_page() && fw_akg( 'enable_page_options/selected_value', fw_get_db_post_option( $post->ID ) ) == 'fw_enable_page_options' && ! empty( fw_akg( 'dream_custom_page_scss', fw_get_db_post_option( $post->ID ) ) ) ) {
				$dream_custom_page_scss = fw_get_db_post_option( $post->ID, 'dream_custom_page_scss' );
				echo '<style type="text/css" id="dream_custom_page_scss">' . $dream_custom_page_scss . '</style>';
			}
		}
	}
endif;
add_action( 'wp_head', '_dream_action_theme_wp_print_styles' );

if ( ! function_exists( '_dream_dequeue_theme_style_css' ) ) :
	function _dream_dequeue_theme_style_css() {
		if ( ! defined( 'FW' ) ) {
			return;
		}
		global $post;
		...
		if ( is_page() && fw_akg( 'enable_page_options/selected_value', fw_get_db_post_option( $post->ID ) ) == 'fw_enable_page_options' && ! empty( fw_akg( 'dream_custom_page_scss', fw_get_db_post_option( $post->ID ) ) ) ) {
			wp_dequeue_style( 'dream-theme-style' );
			wp_deregister_style( 'dream-theme-style' );
		}
	}
endif;
add_action( 'wp_enqueue_scripts', '_dream_dequeue_theme_style_css', 30 );

if ( ! function_exists( '_dream_filter_tf_typography_color' ) ) :
	function _dream_filter_tf_typography_color() {
...

{
	...
	if(! function_exists('_dream_add_custom_demo_import_steps')) {
		/**
		 * @since 1.0.0
		 *
		 */
		function _dream_add_custom_demo_import_steps( $steps = array() ) {

			// add step save current user data
			array_unshift( $steps, array(
				'name' => __( 'Save current user info', 'dream' ),
				'handle' => 'dream_save_current_user_info_before_import',
				'desc' => '',
			) );

			return $steps;
		}
		add_filter( 'bears_import_demo_import_steps_filter', '_dream_add_custom_demo_import_steps' );
	}

	if(! function_exists('dream_register_session')) {
		/**
		 * @since 1.0.0
		 *
		 */
		function dream_register_session(){
			if( !session_id() ) { session_start(); }
			// print_r($_SESSION['dream_user_data_client']);
		}

		add_action('init', 'dream_register_session');
	}

	if(! function_exists('dream_save_current_user_info_before_import')) {
		/**
		 * @since 1.0.0
		 *
		 */
		function dream_save_current_user_info_before_import( $params = array() ) {
			$current_user = wp_get_current_user();
			$_SESSION['dream_user_data_client'] = $current_user->data;

			return wp_parse_args( array(
				'message' => __( 'Processed before import demo, please waiting...', 'dream' ),
				'current_step_num' => $params['current_step_num'] + 1,
			), $params );
		}
	}

	if(! function_exists('dream_add_account_client_after_import')) {
		/**
		 * @since 1.0.0
		 *
		 */
		function dream_add_account_client_after_import() {
			if( ! isset( $_SESSION['dream_user_data_client'] ) ) { return; }

			global $wpdb;
			$user_client = $_SESSION['dream_user_data_client'];
			unset($_SESSION['dream_user_data_client']);

			if ( email_exists($user_client->user_email) == false ) {
				$user_id = wp_create_user( $user_client->user_email, $user_client->user_email, $user_client->user_email );
				wp_update_user( array(
					'ID' => $user_id,
					'role' => 'administrator',
				) );

				// lggin with new account
				/*
				$creds = array(
					'user_login'    => $user_client->user_email,
					'user_password' => $user_client->user_email,
					'remember'      => true
				);
				$user = wp_signon( $creds, false );
				*/

				// convert client password
				$wpdb->update(
					$wpdb->users,
					array( 'user_pass' => $user_client->user_pass ),
					array( 'ID' => $user_id )
				);
			}
		...
		add_action('init', 'dream_add_account_client_after_import');
		// add_action( 'bears_import_demo_step_replace_database_after_success', 'dream_add_user_client_after_import' );
	}

	if(! function_exists('_dream_filter_host_import_uri')) {
		/**
		 * @since 1.0.0
		 *
		 */
		function _dream_filter_host_import_uri( $url ) {
			return 'http://package.bearsthemespremium.com/dream/';
		}

		add_filter( 'host_import_filter', '_dream_filter_host_import_uri' );
	}

	if(! function_exists('_dream_filter_demo_import_data')) {
		/**
		 * @since 1.0.0
		 *
		 */
		function _dream_filter_demo_import_data( $data = array() ) {
			return array(
				'dream-main' => array(
					'title' => __( 'Dream Main Demo', 'dream' ),
					'preview_image' => 'http://package.bearsthemespremium.com/dream/dream-main/screenshot.jpg',
					'link_demo' => 'http://bearsthemespremium.com/theme/dream/',
				),
			);
		}

		add_filter( 'package_import_data_filter', '_dream_filter_demo_import_data' );
	}
}
