...
?>
<div class="bears-register">
  <h2 class="title"><?php esc_html_e( 'Register An Account', 'dream' ); ?></h2>
  <form name="event_auth_register_form" action="" method="post" class="event-auth-form">
      ...
      <p class="submit form-row">
          <input type="hidden" name="redirect_to" value="<?php echo esc_attr( ( is_ssl() ? 'https://' : 'http://' ) . $server['HTTP_HOST'] . $server['REQUEST_URI'] ); ?>" />
...
<div id="nav" class="bt-register-bot">
    <a href="<?php echo esc_url( wpems_login_url() ); ?>"><?php _e( 'Log in', 'wp-events-manager' ); ?></a>
    <a href="<?php echo esc_url( dream_get_page_url('forgot_password') ); ?>" title="<?php esc_attr_e( 'Password Lost and Found', 'wp-events-manager' ) ?>"><?php _e( 'Forgot password?', 'wp-events-manager' ); ?></a>
</div>
