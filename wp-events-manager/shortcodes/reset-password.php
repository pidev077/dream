...
wpems_print_notices();
?>
<div class="bears-reset-password">
  <h2 class="title"><?php esc_html_e( 'Reset Password', 'dream' ); ?></h2>
  <form name="resetpassform" action="<?php echo esc_url( network_site_url( 'wp-login.php?action=resetpass', 'login_post' ) ); ?>" method="POST" class="reset-password event-auth-form">
...
<div id="nav" class="edu-reset-bot">
    <?php if ( !is_user_logged_in() ) : ?>
        <a href="<?php echo esc_url( dream_get_page_url('login') ); ?>"><?php _e( 'Log in', 'wp-events-manager' ); ?></a>
    <?php endif; ?>
    <?php
    if ( get_option( 'users_can_register' ) ) :
        $registration_url = sprintf( '<a href="%s">%s</a>', esc_url( dream_get_page_url('register') ), __( 'Register', 'wp-events-manager' ) );
