...
?>
<h3 class="book-title"><?php esc_html_e( 'Buy Ticket', 'dream' ); ?></h3>
...
<div class="event_register_area">
    ...
    <form name="event_register" class="event_register" method="POST">
        ...
        <ul class="info-event">
            <li>
                <div class="label"><?php esc_html_e( 'Total Slots', 'dream' ); ?></div>
                <div class="value"><?php echo absint( $event->qty ); ?></div>
            </li>
            <li>
                <div class="label"><?php esc_html_e( 'Booked Slots', 'dream' ); ?></div>
                <div class="value"><?php echo esc_html( $event->booked_quantity() ); ?></div>
            </li>
            <li class="event-cost">
                <div class="label"><?php esc_html_e( 'Cost', 'dream' ); ?></div>
                <div class="value"><?php echo ( $event->get_price() ) ? wpems_format_price( $event->get_price() ) . esc_html__( '/Slot', 'dream' ) : '<span class="free">' . esc_html__( 'Free', 'dream' ) . '</span>'; ?></div>
            </li>
            <li>
				<?php if ( $user_reg == 0 && $event->is_free() && wpems_get_option( 'email_register_times' ) === 'once' ) : ?>
                    <div class="label"><?php esc_html_e( 'Quantity', 'dream' ); ?></div>
                    <div class="value">
				...
				<?php else : ?>
                    <div class="label"><?php esc_html_e( 'Quantity', 'dream' ); ?></div>
                    <div class="value">
                        <input type="number" name="qty" value="1" min="1" max="<?php echo esc_attr($event->qty);?>" id="event_register_qty"/>
                    </div>
				<?php endif; ?>
            </li>
			<?php if ( intval( $event->get_price() ) > 0 ) : ?>
                ...
                <li class="event-payment">
                    <div class="label"><?php esc_html_e( 'Pay with', 'dream' ); ?></div>
        ...
        <?php if(empty($payments)) { ?>
            <p class="event_auth_register_message_error">
                <?php echo esc_html__( 'You must set payment setting!', 'dream' ); ?>
            </p>
        <?php }?>
        ...
        <div class="event_register_foot">
			...
			<?php if ( $is_expired ) : ?>
                <button type="submit" disabled class="event_button_disable"><?php esc_html_e( 'Expired', 'dream' ); ?></button>
			<?php elseif ( absint( $event->qty ) == 0 ) : ?>
                <button type="submit" disabled class="event_button_disable"><?php esc_html_e( 'Sold Out', 'dream' ); ?></button>
			<?php else : ?>
				<?php if ( ! is_user_logged_in() ) { ?>
                    <a href="<?php echo esc_url( add_query_arg( 'redirect_to', urlencode( get_permalink() ), wp_login_url() ) ); ?>" class="event_register_submit event_auth_button"><?php esc_html_e( 'Login Now', 'dream' ); ?></a>
                    <p></p>
                    <p class="login-notice">
						<?php echo esc_html__( 'You must login to our site to book this event!', 'dream' ); ?>
                    </p>
				<?php } else { ?>
                    <button type="submit" class="event_register_submit event_auth_button" <?php echo esc_attr($payments) ? '' : 'disabled="disabled"' ?>><?php esc_html_e( 'Book Now', 'dream' ); ?></button>
				<?php } ?>
			<?php endif ?>
