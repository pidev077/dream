...
if( ! function_exists('dream_get_all_taxomony_portfolio') ) :
	function dream_get_all_taxomony_portfolio($params = array()) {
...
if(! function_exists('dream_builder_filter_taxonomy_portfolio')) :
	/**
	 * dream_builder_filter_taxonomy_portfolio
	 *
	 * @param [array] 		$params
	 * @param [boolean] 	$filter_masonry
	 */
	function dream_builder_filter_taxonomy_portfolio($params = array(), $filter_masonry = false) {
		$terms = dream_get_all_taxomony_portfolio();
		...
		if($filter_masonry == true){
			echo '<a href="#" data-filter="*" class="portfolio-filter-item is-active">'. esc_html__('All', 'dream') .'</a>';
		}
				...
				foreach ( $terms as $term ) {
					...
					if ( $template_parent != $term->parent ) {
						if ( $term->slug == $template_slug ) {
							$dream_permalink     = get_term_link( $term->slug, $taxonomy );
							$icon          = '';
							$category_icon = fw_get_db_term_option( $term->term_id, $taxonomy, 'category_icon', '' );
							if ( $category_icon != '' ) {
								$icon = '<i class="' . $category_icon . '"></i>';
							}
							$term_view_all .= '<li class="categories-item active" data-category="' . $template_slug . '"><a href="' . $dream_permalink . '">' . $icon . dream_translate($term->name) . '</a></li>';
							...
							$term_view_all .= '<li class="categories-item" data-category="' . $term->slug . '"><a href="' . $dream_permalink . '">' . $icon . dream_translate($term->name) . '</a></li>';
						}
						...
						if ( $term->slug == $template_slug ) {
							...
							$term_list .= '<li class="categories-item active" data-category="' . $template_slug . '"><a href="' . $dream_permalink . '">' . $icon . dream_translate($term->name) . '</a></li>';
						} else {
							$dream_permalink     = get_term_link( $term->slug, $taxonomy );
							$icon          = '';
							$category_icon = fw_get_db_term_option( $term->term_id, $taxonomy, 'category_icon', '' );
							...
							$term_list .= '<li class="categories-item" data-category="' . $term->slug . '"><a href="' . $dream_permalink . '">' . $icon . dream_translate($term->name) . '</a></li>';
						}
					}
				}
...
if ( ! function_exists( 'dream_get_portfolio' ) ):
	/**
	 ...
	 */
	function dream_get_portfolio( $args = null ) {
		$defaults = array(
