...
if ( ! is_admin() ) {
	...
	if ( is_tax( $settings['taxonomy_name'] ) || is_post_type_archive( $settings['post_type'] ) ) {
		...
		wp_register_script( 'general', $dream_template_directory . '/js/general.js', array( 'jquery' ) );
		// localize ajax_url
		wp_localize_script( 'general', 'BtPhpVars', array(
			'ajax_url'       => admin_url( 'admin-ajax.php' ),
			'paged'          => $paged,
			'max_num_pages'  => $max_num_pages,
			'loading_text'   => esc_html__( 'Loading ...', 'dream' ),
			'load_more_text' => esc_html__( 'Load More', 'dream' ),
