 ...
 *    @var $post_excerpt
 */
$posts_data = dream_get_posts($args);
  ...
  <div class="vc-custom-inner-wrap">
	...
	if(count($posts_data) > 0):
		...
		$temp_variables = array(
			// '{image_html}'    => '',
			'{pid}'						=> $item['post_id'],
			'{readmore_html}' => '<a href="'. $item['post_link'] .'" class="post-view-detail">'. __('Read More', 'dream') .' <span class="ion-ios-arrow-thin-right"></span></a>',
		...
		echo sprintf('<div class="item">%s</div>', __('There are no posts to display!', 'dream'));
	endif;
	?>
    </div>
  </div>
