...
<article id="post-<?php the_ID(); ?>" <?php post_class( "post post-details" ); ?> itemscope="itemscope" itemtype="http://schema.org/BlogPosting" itemprop="blogPost">
	<div class="fw-col-inner">
        ...
        <!-- Post audio -->
    		<?php if(!empty($dream_post_options['audio'])) { ?>
    			<div class="post-block-audio-wrap" style="background: url(<?php echo esc_attr($dream_post_thumbnail_url); ?>) no-repeat center center / cover;">
    				<?php echo "{$dream_post_options['audio']}"; ?>
    			</div>
    		<?php } ?>
			...
			if(isset($dream_post_options['tag_list']) && ! empty($dream_post_options['tag_list'])) {
				echo "<div class='single-entry-tag'>". __('Tags: ', 'dream') . "{$dream_post_options['tag_list']}</div>";
			}
